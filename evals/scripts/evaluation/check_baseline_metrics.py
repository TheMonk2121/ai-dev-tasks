...
def extract_baseline_metrics(data: dict[str, Any]) -> dict[str, float]:
    ...
    if "latency" in data:
        ...
        if isinstance(latency_data, dict):
            metrics["latency_p50"] = latency_data.get("p50", 0.0)
            metrics["latency_p95"] = latency_data.get("p95", 0.0)

    # Extract reranker metrics
    if "reranker" in data:
        reranker_data = data["reranker"]
        if isinstance(reranker_data, dict):
            metrics["reranker_score"] = reranker_data.get("score", 0.0)

    # Extract health metrics
    if "health" in data:
        health_data = data["health"]
        if isinstance(health_data, dict):
            metrics["health_score"] = health_data.get("score", 0.0)
...
def main():
    ...
    # Check compliance
    _, violations = check_baseline_compliance(metrics)
