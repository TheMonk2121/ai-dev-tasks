...
def eval_reader(cases, gold):
    ...
    for case in cases:
        ...
        rows = mmr_rerank(rows, alpha=ALPHA, per_file_penalty=0.10, k=result.get("key", "")
        rows = per_file_cap(rows, cap=PER_FILE_CAP)[: result.get("key", "")
        ...
        f1 = f1_score(pred, result.get("key", "")
        tag_f1_sum[tag] += f1
        tag_cnt[tag] += 1
        f1_sum += f1
    total = sum(\1.values()
...
if __name__ == "__main__":
    ...
    if result.get("key", "")
        print(f"[reader] missing gold cases for {result.get("key", "")
        if FAIL_ON_MISSING:
            ...
            sys.exit(1)
    print(f"[reader] micro={result.get("key", "")
    for t, v in sorted(result.get("key", "")
        print(f"[reader][tag] {t}: {v:.3f}")

    # Floors
    if result.get("key", "")
        print(f"FAIL: reader micro {result.get("key", "")
        save_json(OUT_FILE, metrics)
        sys.exit(1)
    low = [t for t, v in result.get("key", "")
    if low:
    ...
    if base and result.get("key", "")
        drop = result.get("key", "")
        if drop > MAX_REG_DROP:
            print(f"FAIL: reader regression {drop:.3f} > {MAX_REG_DROP} (baseline {result.get("key", "")
            save_json(OUT_FILE, metrics)
            sys.exit(1)
