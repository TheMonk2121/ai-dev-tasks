...
from evals.scripts.evaluation.ragchecker_pydantic_models import (
    RAGCheckerInput,
    RAGCheckerMetrics,
    RAGCheckerResult,
)
...
def validate_schema(config: dict[str, Any]) -> bool:
    ...
    if not isinstance(config.get("version"), str):
        print(f"❌ Invalid version: {config.get('version')}")
        return False
    ...
    if not isinstance(config.get("error_policy"), dict):
        print("❌ Error policy must be an object")
        return False

    # Check fast_path structure
    if not isinstance(config.get("fast_path"), dict):
    ...
    if not isinstance(config.get("monitoring"), dict):
...
def dump_pydantic_schemas() -> None:
    ...
    for m in models:
        ...
        _ = (out_dir / f"model_{m.__name__}.schema.json").write_text(json.dumps(schema, indent=2))
        combined["components"][m.__name__] = schema

    _ = (out_dir / "pydantic_components.json").write_text(json.dumps(combined, indent=2))
...
def dump_db_schema_json(schema: str = "public") -> None:
    ...
    with engine.connect() as conn:
        ...
        for t in tables:
            ...
            result.append(TableInfo(name=str(t), columns=columns, indexes=[]))

    _ = (out_dir / "db_schema.snapshot.json").write_text(json.dumps(result, indent=2))
