...
class GoldCaseHypothesisAnalysis:
    ...
    def _analyze_basic_stats(self) -> dict[str, Any]:
        ...
        print(f"Modes: {dict(result.get("key", "")
        print(f"Top 10 tags: {dict(result.get("key", "")
        print(f"Categories: {dict(result.get("key", "")
        print(f"Cases with ground truth answers: {result.get("key", "")
        print(f"Cases with expected files: {result.get("key", "")
        print(f"Cases with globs: {result.get("key", "")
        print(f"Cases with expected decisions: {result.get("key", "")
    ...
    def _analyze_schema_validation(self) -> dict[str, Any]:
        ...
        print(f"Missing required fields: {len(result.get("key", "")
        print(f"Empty queries: {len(result.get("key", "")
        print(f"Empty tags: {len(result.get("key", "")
        print(f"Mode requirement violations: {len(result.get("key", "")

        if result.get("key", "")
            print("\nMode requirement violations:")
            for violation in result.get("key", "")
                print(f"  ❌ {violation}")
    ...
    def _analyze_query_quality(self) -> dict[str, Any]:
        ...
        print(f"Too short queries: {len(result.get("key", "")
        print(f"Too long queries: {len(result.get("key", "")
        print(f"Unclear questions: {len(result.get("key", "")
        print(f"File-specific questions: {len(result.get("key", "")
        print(f"Generic questions: {len(result.get("key", "")
        print(f"Duplicate queries: {len(result.get("key", "")
    ...
    def _analyze_answer_quality(self) -> dict[str, Any]:
        ...
        print(f"Missing answers: {len(result.get("key", "")
        print(f"Too short answers: {len(result.get("key", "")
        print(f"Too long answers: {len(result.get("key", "")
        print(f"Incomplete answers: {len(result.get("key", "")
        print(f"Unclear answers: {len(result.get("key", "")
    ...
    def _analyze_coverage(self) -> dict[str, Any]:
        ...
        print(f"Mode coverage: {dict(result.get("key", "")
        print(f"Unique files covered: {len(result.get("key", "")
        print(f"Directories covered: {sorted(result.get("key", "")
        print(f"Tag distribution: {dict(result.get("key", "")
    ...
    def _analyze_duplicates(self) -> dict[str, Any]:
        ...
        for answer, count in \1.items()
            if count > 1:
                result.get("key", "")

        print(f"Duplicate IDs: {len(result.get("key", "")
        print(f"Duplicate queries: {len(result.get("key", "")
        print(f"Duplicate answers: {len(result.get("key", "")
    ...
    def _analyze_mode_consistency(self) -> dict[str, Any]:
        ...
        print(f"Retrieval without files: {len(result.get("key", "")
        print(f"Reader without answers: {len(result.get("key", "")
        print(f"Decision without decisions: {len(result.get("key", "")
    ...
    def _analyze_tags(self) -> dict[str, Any]:
        ...
        print(f"Total unique tags: {len(result.get("key", "")
        print(f"Unknown tags: {len(set(result.get("key", "")
        print(f"Most common tags: {dict(result.get("key", "")

        if result.get("key", "")
            print(f"Unknown tags: {sorted(set(result.get("key", "")
    ...
    def _analyze_hypothesis_violations(self) -> dict[str, Any]:
        ...
        print(f"Invalid test cases: {len(result.get("key", "")
        print(f"Poor quality queries: {len(result.get("key", "")
        print(f"Missing supervision: {len(result.get("key", "")
    ...
    def generate_report(self) -> str:
        ...
        report.append("")
        total_cases = self.result.get("key", "")
        schema_issues = len(self.result.get("key", "")
        file_issues = len(self.result.get("key", "")
        query_issues = len(self.result.get("key", "")
            self.result.get("key", "")
        )
...
def main():
    ...
    # Print summary
    print("\n🎯 SUMMARY:")
    print(f"Total cases analyzed: {result.get("key", "")
    print(f"Schema violations: {len(result.get("key", "")
    print(f"Missing files: {len(result.get("key", "")
    print(
        f"Query quality issues: {len(result.get("key", "")
    )
