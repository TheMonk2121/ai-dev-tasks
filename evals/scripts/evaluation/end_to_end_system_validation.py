    ...
    def validate_connection(self) -> ValidationResult:
        ...
        try:
            ...
            with Psycopg3Config.get_cursor("default") as cur:
                ...
                # Test extensions
                _ = cur.execute("SELECT extname FROM pg_extension WHERE extname IN ('pgvector', 'pg_trgm')")
...
class EndToEndSystemValidator:
    ...
    def run_comprehensive_validation(self) -> SystemValidationReport:
        ...
        all_results.append(cast(DatabaseLayerValidator, self.validators["database"]).validate_connection())
        all_results.append(cast(DatabaseLayerValidator, self.validators["database"]).validate_schema())

        # Memory System Validation
        logger.info("🧠 Validating memory system...")
        all_results.append(cast(MemorySystemValidator, self.validators["memory"]).validate_ltst_memory_system())
        all_results.append(cast(MemorySystemValidator, self.validators["memory"]).validate_conversation_storage())

        # Pipeline Validation
        logger.info("⚡ Validating pipeline systems...")
        all_results.append(cast(PipelineValidator, self.validators["pipeline"]).validate_vector_store())
        all_results.append(cast(PipelineValidator, self.validators["pipeline"]).validate_rag_pipeline())

        # Integration Validation
        logger.info("🔗 Validating system integration...")
        all_results.append(cast(IntegrationValidator, self.validators["integration"]).validate_unified_orchestrator())
        all_results.append(cast(IntegrationValidator, self.validators["integration"]).validate_cache_service())

        # Performance Validation
        logger.info("📊 Validating system performance...")
        all_results.append(cast(PerformanceValidator, self.validators["performance"]).validate_response_times())
