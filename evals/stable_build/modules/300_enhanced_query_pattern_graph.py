...
class EnhancedQueryPatternGraph:
    ...
    def add_query_pattern(self, query: str, intent: str, context: dict[str, Any]) -> str:
        ...
        query_node = SemanticNode(
            ...
            content=query,
            metadata={"original_query": query, "timestamp": datetime.now().isoformat()}
        )
        ...
        intent_node = SemanticNode(
            ...
            content=intent,
            metadata={"intent_type": "user_query"}
        )
    ...
    def _apply_syntactic_augmentation(self, pattern_id: str) -> str:
        ...
        for source, target, data in edges_to_copy:
            ...
            self.graph.add_edge(
                new_source,
                new_target,
                **{**data, "weight": modified_weight, "metadata": {**data.get("metadata", {}), "augmented": True}}
            )
    ...
    def _apply_semantic_augmentation(self, pattern_id: str) -> str:
        ...
        for node_id in pattern_nodes:
            ...
            self.graph.add_node(
                new_node_id,
                **{**original_data, "metadata": {**original_data.get("metadata", {}), "semantic_augmented": True}}
            )
    ...
    def export_graph(self) -> dict[str, Any]:
        ...
        return {
            ...
            "metadata": {
                ...
                "node_types": list(set([self.graph.nodes[n]["node_type"] for n in self.graph.nodes()])),
                "edge_types": list(set([self.graph.edges[e]["edge_type"] for e in self.graph.edges()]))
            },
        }
