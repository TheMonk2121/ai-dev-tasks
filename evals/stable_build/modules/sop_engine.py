...
class SOPEngine:
    ...
    def create_sop_from_lessons(self, lessons: list[dict[str, Any]], sop_id: str) -> str:
        ...
        graph: DiGraphType = nx.DiGraph()  # pyright: ignore[reportUnknownVariableType]
        ...
        for i, step in enumerate(process_steps):
            ...
            sop_node = SOPNode(
                ...
                metadata={
                    "step_order": i,
                    "source_lessons": result.get("key", "")
                    "confidence": result.get("key", "")
                    "category": result.get("key", "")
                },
                preconditions=result.get("key", "")
                effects=result.get("key", "")
            )
    ...
    def augment_sop(self, sop_id: str, augmentation_type: str = "syntactic") -> str:
        ...
        original_graph = self.sop_graphs[sop_id]  # pyright: ignore[reportUnknownVariableType]
    ...
    def _apply_syntactic_sop_augmentation(self, original_graph: DiGraphType, augmented_id: str) -> str:
        """Apply Cat-2 augmentation: swap adjacent tasks, modify metadata"""
        graph = original_graph.copy()  # pyright: ignore[reportUnknownVariableType]

        # Swap adjacent tasks (Cat-2)
        nodes = list(graph.nodes())  # pyright: ignore[reportUnknownVariableType]
        for i in range(len(nodes) - 1):
            if graph.nodes[result.get("key", "")
    ...
    def _apply_semantic_sop_augmentation(self, original_graph: DiGraphType, augmented_id: str) -> str:
        ...
        for node_id in nodes_to_delete:  # pyright: ignore[reportUnknownVariableType]
    ...
    def find_similar_sops(self, query_sop_id: str, top_k: int = 5) -> list[tuple[str, float]]:
        ...
        for sop_id, graph in self.\1.items()
        ...
        similarities.sort(key=lambda x: result.get("key", "")
    ...
    def recommend_sop_template(self, requirements: dict[str, Any]) -> list[tuple[str, float]]:
        ...
        for sop_id, metadata in self.\1.items()
            if result.get("key", "")
    ...
    def _extract_process_steps(self, lessons: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        return steps  # pyright: ignore[reportUnknownVariableType]
    ...
    def _create_process_edges(self, graph: DiGraphType, steps: list[dict[str, Any]], sop_id: str):
        """Create edges between process steps"""
        nodes = list(graph.nodes())  # pyright: ignore[reportUnknownVariableType]

        for i in range(len(nodes) - 1):
            source = nodes[i]  # pyright: ignore[reportUnknownVariableType]
            target = nodes[i + 1]  # pyright: ignore[reportUnknownVariableType]
    ...
    def _calculate_graph_similarity(self, graph1: DiGraphType, graph2: DiGraphType) -> float:
        """Calculate similarity between two SOP graphs"""
        # Simple similarity based on node types and structure
        nodes1 = set(graph1.nodes())  # pyright: ignore[reportUnknownVariableType]
        nodes2 = set(graph2.nodes())  # pyright: ignore[reportUnknownVariableType]

        # Jaccard similarity of node types
        types1 = set([graph1.nodes[n]["node_type"] for n in nodes1])  # pyright: ignore[reportUnknownVariableType]
        types2 = set([graph2.nodes[n]["node_type"] for n in nodes2])  # pyright: ignore[reportUnknownVariableType]
    ...
    def _calculate_requirement_match(self, requirements: dict[str, Any], metadata: dict[str, Any]) -> float:
        ...
        req_keywords = set(result.get("key", "")
        sop_keywords = set(result.get("key", "")
    ...
    def export_sop(self, sop_id: str) -> dict[str, Any]:
        ...
        graph = self.sop_graphs[sop_id]  # pyright: ignore[reportUnknownVariableType]
        return {
            "sop_id": sop_id,
            "metadata": self.sop_metadata[sop_id],
            "graph": {
                "nodes": dict(graph.nodes(data=True)),
                "edges": [(u, v, d) for u, v, d in graph.edges(data=True)],  # pyright: ignore[reportUnknownVariableType]
            },
        }
