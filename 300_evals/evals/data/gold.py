...
DEPRECATED: This legacy module supported older gold formats (300_evals/data/gold.jsonl,
300_evals/data/gold_cases.json). The canonical dataset now lives at
`300_evals/data/gold/v1/gold_cases.jsonl` and should be accessed via
`src.utils.gold_loader` and associated helpers. This module remains for
backwards compatibility with older scripts but should not be used for
new work.
"""
...
def load_gold_mapping(path: str | None = None) -> dict[str, dict[str, Any]]:
    ...
    path = path or os.getenv("GOLD_FILE", "300_evals/data/gold.jsonl")
    if os.path.exists("300_evals/data/gold/v1/gold_cases.jsonl") and not os.path.exists(path):
        # Gentle nudge if the legacy default doesn't exist but v1 does
        warnings.warn(
            "Legacy gold file not found. Consider using 300_evals/data/gold/v1/gold_cases.jsonl",
            DeprecationWarning,
            stacklevel=2,
        )
