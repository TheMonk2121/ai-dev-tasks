...
def annotate_item(item: Dict[str, Any]) -> Dict[str, Any]:
    ...
    mode = (updated.get("mode") or "").lower()

    # Ensure assertions container
    assertions: Dict[str, Any] = to_dict(updated.get("assertions"))

    def mark_scored(flag: bool) -> None:
        updated["scored"] = bool(flag)

    # Decision mode
    if mode == "decision":
        expected = to_list(updated.get("expected_decisions"))
        if expected:
    ...
    # Retrieval mode
        ...
        expected_files = to_list(updated.get("expected_files")) or to_list(updated.get("gold_doc_ids"))
        globs = to_list(updated.get("globs"))
        if expected_files:
            k = 5
    ...
    # Reader mode
        ...
        gt_answer = updated.get("gt_answer")
        expected_answers = to_list(updated.get("expected_answers"))
    ...
    if mode == "reader" and (updated.get("expected_files") or updated.get("required_sources")):
...
def main() -> None:
    ...
    parser.add_argument(
        "--path",
        default="300_evals/evals/data/gold/v1/gold_cases.jsonl",
