...
class Phase2RAGSystem:
    ...
    def __init__(
        self,
        base_retriever,
        multihop_config: dict[str, Any] | None = None,
    ...
    async def query_with_multihop(
        ...
        try:
            ...
            if should_use_multihop:
                ...
                stage_start = time.time()
                multihop_result = await self._execute_multihop_planning(query, sub_claims, request_id)
                stage_timings["multihop_planning"] = (time.time() - stage_start) * 1000

                # Stage 3: Evidence synthesis and answer generation
                stage_start = time.time()
                final_answer = await self._synthesize_multihop_answer(query, multihop_result, request_id)
                ...
                result = {**single_hop_result, "multihop_used": False, "complexity_analysis": complexity}
    ...
    def _assess_query_complexity(self, query: str) -> dict[str, Any]:
        ...
        has_comparisons = any(
            comp in query.lower() for comp in ["compare", "difference", "versus", "vs", "traditional"]
        )
    ...
    async def _execute_multihop_planning(
        self, query: str, sub_claims: list[str] | None, request_id: str | None
    ) -> dict[str, Any]:
        ...
        multihop_result = await self.multihop_planner.plan_multihop_retrieval(
            query=query, retrieval_fn=retrieval_fn, sub_claims=sub_claims
        )

        return multihop_result

    async def _synthesize_multihop_answer(
        self, query: str, multihop_result: dict[str, Any], request_id: str | None
    ) -> dict[str, Any]:
        ...
        if self.enable_midgen_callbacks and base_confidence < self.midgen_uncertainty_threshold and len(evidence) > 0:

            enhanced_answer = await self._midgen_callback_enhancement(query, base_answer, evidence, request_id)
    ...
    async def _midgen_callback_enhancement(
        self, query: str, base_answer: str, evidence: list[dict[str, Any]], request_id: str | None
    ) -> dict[str, Any]:
        ...
        enhanced_text = self._enhance_answer_with_evidence(base_answer, additional_evidence, unresolved_spans)
    ...
    def _identify_unresolved_spans(self, answer: str, evidence: list[dict[str, Any]]) -> list[str]:
        ...
        content_words = [word for word in unsupported if len(word) > 3 and word.isalpha()]

        return content_words[:5]  # Top 5 unresolved spans

    def _enhance_answer_with_evidence(
        self, base_answer: str, additional_evidence: list[dict[str, Any]], unresolved_spans: list[str]
    ) -> str:
    ...
    async def _execute_single_hop(self, query: str, request_id: str | None) -> dict[str, Any]:
    ...
    async def _log_phase2_request(self, query: str, result: dict[str, Any], request_id: str | None) -> None:
...
def create_phase2_rag_system(base_retriever, config: dict[str, Any] | None = None) -> Phase2RAGSystem:
