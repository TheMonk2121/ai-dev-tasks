...
def analyze_results(results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    analysis = {
        ...
        "successful_passes": sum(1 for r in results if r.get("status") == "success"),
        "failed_passes": sum(1 for r in results if r.get("status") != "success"),
        "pass_criteria": {
    ...
    for result in results:
        if result.get("status") == "success":
            print(f"✅ {result.get('name', 'Unknown')} passed")
        else:
            print(f"❌ {result.get('name', 'Unknown')} failed")
...
def main():
    ...
    for i, pass_config in enumerate(passes, 1):
        pass_name = pass_config.get("name", f"pass_{i}")
        output_file = results_dir / f"pass_{i}_{pass_name}.json"

        print(f"\n📋 PASS {i}: {pass_name}")
        print(f"   Config: {pass_config}")

        result = run_evaluation_pass(pass_name, pass_config, str(output_file))
