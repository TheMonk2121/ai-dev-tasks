...
class TestLTSTIntegrationCore(unittest.TestCase):
    ...
    def test_02_context_merging_functions(self):
        ...
        merge_result = self.ltst_system.merge_contexts_database(
            self.existing_session, relevance_threshold=0.1
        )
        ...
        print(f"✓ Context merging: {merge_result.source_context_count} contexts, "
    ...
    def test_03_memory_rehydration_functions(self):
        ...
        rehydration_result = self.ltst_system.rehydrate_memory_database(
            self.existing_session, self.existing_user
        )
        ...
        print(f"✓ Memory rehydration: {rehydration_result.continuity_score:.3f} continuity, "
    ...
    def test_04_session_continuity_detection(self):
        ...
        # Test with existing session
        continuity_info = self.ltst_system.database_integration.get_session_continuity(
        ...
        print(f"✓ Session continuity: {continuity_info['continuity_score']:.3f} score, "
    ...
    def test_06_error_handling(self):
        ...
        try:
            rehydration_result = self.ltst_system.rehydrate_memory_database(
                self.existing_session, "invalid_user_12345"
            )
        ...
        try:
            merge_result = self.ltst_system.merge_contexts_database(
                self.existing_session, relevance_threshold=1.0
            )
            self.assertIsNotNone(merge_result)
