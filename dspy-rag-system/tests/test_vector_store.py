...
import sys
sys.path.append('src')
from src.dspy_modules.vector_store import VectorStore, _get_pool, _get_model

# Test database configuration
POSTGRES_DSN = os.getenv("PG_DSN_TEST", 
...
def store(db_conn):
    ...
    return VectorStore(POSTGRES_DSN)
...
def test_singleton_model():
    ...
    vs1 = VectorStore(POSTGRES_DSN)
    vs2 = VectorStore(POSTGRES_DSN)
...
def test_connection_pool_singleton():
    ...
    pool1 = _get_pool(POSTGRES_DSN)
    pool2 = _get_pool(POSTGRES_DSN)
...
def test_embedding_generation():
    ...
    store = VectorStore(POSTGRES_DSN)
...
def test_database_connection_failure():
    ...
    invalid_store = VectorStore("postgresql://invalid:invalid@localhost:9999/invalid")
