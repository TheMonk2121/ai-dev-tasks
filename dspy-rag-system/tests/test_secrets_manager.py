...
from utils.secrets_manager import (
    SecretsManager, SecretConfig, validate_startup_secrets,
    setup_secrets_interactive, SecretsError, SecretsValidationError
)
...
class TestSecretsManager:
    ...
    def test_create_secrets_config(self):
        ...
        expected_secrets = [
            "POSTGRES_DSN", "OLLAMA_BASE_URL", "DASHBOARD_SECRET_KEY",
            "API_KEY", "DB_PASSWORD", "REDIS_URL", "METRICS_PORT"
        ]
        ...
        ollama_config = next(c for c in configs if c.name == "OLLAMA_BASE_URL")
        assert ollama_config.required is False
        assert ollama_config.default == "http://localhost:11434"
