...
from dspy_modules.cursor_model_router import create_cursor_model_router, CursorModel
...
def test_context_engineering():
    ...
    if successful_tests:
        ...
        print(f"Model selection accuracy: {len(matches)}/{len(successful_tests)} ({len(matches)/len(successful_tests)*100:.1f}%)")
