...
class RelationExtractor(Module):
    ...
    def forward(self, text: str, relation_types: List[str]) -> Dict[str, Any]:
        ...
        # Research-based assertions for validation
        dspy.Assert(self.validate_relations(result.relations), "Relations must be valid")
        dspy.Assert(self.validate_spans(result.spans, text), "Spans must be valid")
        dspy.Assert(0 <= result.confidence <= 1, "Confidence must be between 0 and 1")
...
class FactExtractor(Module):
    ...
    def forward(self, text: str, fact_schema: ExtractionSchema) -> Dict[str, Any]:
        ...
        # Research-based assertions for validation
        dspy.Assert(self.validate_facts(result.facts, fact_schema), "Facts must match schema")
        dspy.Assert(self.validate_spans(result.spans, text), "Spans must be valid")
        dspy.Assert(0 <= result.confidence <= 1, "Confidence must be between 0 and 1")
