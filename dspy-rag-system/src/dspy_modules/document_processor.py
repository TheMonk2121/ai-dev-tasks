...
import sys
sys.path.append('src')
from utils.logger import get_logger, log_document_processing, log_error_with_context
...
class DocumentProcessor(Module):
    ...
    def forward(self, document_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info("Extracting text from document", extra={
                'document_id': document_id,
            ...
            self.logger.info("Creating chunks using token-aware chunking", extra={
                'document_id': document_id,
                'stage': 'chunking'
            })
            ...
            self.logger.info("Extracting metadata using config-driven rules", extra={
                'document_id': document_id,
            ...
            self.logger.info("Document processing completed successfully", extra={
                'document_id': document_id,
                'stage': 'complete',
            ...
            self.logger.exception("Document processing failed", extra={
                'document_id': document_id,
                'stage': 'error',
                'error': str(e),
                'file_path': self._mask_file_path(document_path)
        ...
        try:
            ...
            for page_num, page in enumerate(doc):
                ...
                if page_text:
                    ...
                    self.logger.warning(f"Page {page_num + 1} has no extractable text", extra={
                        'file_path': self._mask_file_path(pdf_path),
                        'page_num': page_num + 1
                    })
            ...
            self.logger.error(f"Error reading PDF file: {e}", extra={
                'file_path': self._mask_file_path(pdf_path),
    ...
    def _extract_text_file(self, file_path: str) -> str:
        ...
        try:
            ...
            try:
                ...
                self.logger.error(f"Error reading text file: {e}", extra={
                    'file_path': self._mask_file_path(file_path)
                })
    ...
    def _extract_csv_text_pandas(self, csv_path: str) -> str:
        ...
        try:
            ...
            for i, row in df.head(10).iterrows():
                row_text = f"Row {i+1}: " + ", ".join([f"{col}={val}" for col, val in row.items()])
            ...
            self.logger.warning(f"Pandas CSV extraction failed: {e}", extra={
                'file_path': self._mask_file_path(csv_path)
            })
            raise
    ...
    def _extract_csv_text_streaming(self, csv_path: str) -> str:
        ...
        try:
            ...
            text_parts.append("Sample data:")
            for i, row in first_chunk.head(10).iterrows():
                row_text = f"Row {i+1}: " + ", ".join([f"{col}={val}" for col, val in row.items()])
                text_parts.append(row_text)
            ...
            if len(numeric_cols) > 0:
                ...
                for col in numeric_cols:
                    text_parts.append(f"{col}: min={first_chunk[col].min()}, max={first_chunk[col].max()}, mean={first_chunk[col].mean():.2f}")
            
            return "\n".join(text_parts)
            
        except Exception as e:
            self.logger.warning(f"Streaming CSV extraction failed: {e}", extra={
                'file_path': self._mask_file_path(csv_path)
            })
            raise
    ...
    def _extract_csv_text_basic(self, csv_path: str) -> str:
        ...
        try:
            ...
            self.logger.error(f"Error reading CSV file: {e}", extra={
                'file_path': self._mask_file_path(csv_path)
            })
...
class DocumentIngestionPipeline(Module):
    ...
    def forward(self, document_path: str, vector_store: Any = None) -> Dict[str, Any]:
        ...
        try:
            self.logger.info("Starting document ingestion pipeline", extra={
            ...
            if vector_store and hasattr(vector_store, 'store_chunks'):
                try:
                    ...
                    self.logger.info("Chunks stored in vector database", extra={
                        'document_id': result.get('document_id', 'unknown'),
                        'chunks_stored': result["total_chunks"]
                    })
                except Exception as e:
                    self.logger.error("Failed to store chunks in vector database", extra={
                        'document_id': result.get('document_id', 'unknown'),
                        'error': str(e)
                    })
            ...
            self.logger.info("Document ingestion pipeline completed", extra={
                'document_id': result.get('document_id', 'unknown'),
            ...
            self.logger.exception("Document ingestion pipeline failed", extra={
                'file_path': self.processor._mask_file_path(document_path),
