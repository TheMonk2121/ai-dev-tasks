...
class ToolTrapManager:
    ...
    def register_tool(self, schema: ToolSchema) -> None:
        ...
        self.circuit_breakers[schema.name] = CircuitBreaker(
            threshold=schema.circuit_breaker_threshold
        )
        ...
        for param, value in tool_call.parameters.items():
            if param in properties:
                ...
                if expected_type and not self._check_type(value, expected_type):
                    issues.append(f"Parameter '{param}' has wrong type. Expected {expected_type}, got {type(value).__name__}")
