...
class CITestRunner:
    ...
    def _detect_ci_environment(self) -> bool:
        ...
        ci_vars = ["CI", "TRAVIS", "CIRCLECI", "GITHUB_ACTIONS", "GITLAB_CI", "JENKINS_URL", "BUILD_ID"]
    ...
    def _run_category_tests(self, category: str):
        ...
        try:
            ...
            cmd = [
                ...
                "--coverage-threshold",
                str(self.config["coverage_threshold"]),
            ]
            ...
            self.test_results[category] = {"category": category, "success": False, "error": "Test execution timed out"}
        except Exception as e:
            logger.error(f"❌ {category} tests failed: {e}")
            self.test_results[category] = {"category": category, "success": False, "error": str(e)}
    ...
    def _handle_ci_failure(self, error: Exception):
        ...
        # Save failure report
        failure_report = {"error": str(error), "timestamp": datetime.now().isoformat(), "ci_platform": self.ci_platform}
