#!/usr/bin/env python3
"""
Simple bulk document processing using direct database connections.
This bypasses the DatabaseResilienceManager pool that has KeyError: 0 issues.
"""
...
def process_document_simple(doc_info, conn):
    """Process a single document using direct database connection"""
    try:
...
def main():
    ...
    print("=== Simple Bulk Document Processing ===")
    ...
    for i, doc in enumerate(documents, 1):
        ...
        if process_document_simple(doc, conn):
            successful += 1
        else:
            failed += 1
