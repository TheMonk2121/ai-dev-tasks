...
from utils.logger import setup_logger, log_document_processing
...
def add_document_to_rag(file_path):
    ...
    logger.info(f"Starting document addition", extra={
    ...
    try:
        ...
        if not os.path.exists(file_path):
            logger.error(f"File not found", extra={
                'document_id': document_id,
                'stage': 'error',
                'file_path': file_path
            })
        ...
        logger.info(f"Enhanced metadata extracted", extra={
            'document_id': document_id,
            'stage': 'metadata',
        ...
        logger.info(f"Storing in vector database", extra={
            'document_id': document_id,
            'stage': 'storage'
        })
        ...
        if store_result['status'] == 'success':
            ...
            logger.error(f"Failed to store document", extra={
                'document_id': document_id,
                'stage': 'storage_error',
        ...
        logger.error(f"Error processing document", extra={
            'document_id': document_id,
            'stage': 'error',
            'error': str(e)
        })
...
def query_knowledge_base(query, limit=3):
    ...
    logger.info(f"Querying knowledge base", extra={
        'query': query,
        'limit': limit,
        'stage': 'query_start'
    })
    ...
    try:
        ...
        if results['status'] == 'success':
            ...
            logger.error(f"Search failed", extra={
                'query': query,
                'stage': 'query_error',
        ...
        logger.error(f"Error querying knowledge base", extra={
            'query': query,
            'stage': 'query_error',
            'error': str(e)
        })
