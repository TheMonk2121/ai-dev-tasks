...
class DocumentationAgent(BaseAgent):
    ...
    def can_handle(self, request: AgentRequest) -> bool:
        ...
        documentation_keywords = [
            ...
            "readme", "api", "tutorial", "guide", "help"
        ]
...
def _env_bool(name: str, default: bool) -> bool:
    ...
    async def process_request(self, request: AgentRequest) -> AgentResponse:
        ...
        try:
            ...
            if best_agent != self.active_agent:
    ...
    async def _select_best_agent(self, request: AgentRequest) -> BaseAgent:
        ...
        if "research" in request.query.lower():
            return self.agents[AgentType.RESEARCH]
        elif "code" in request.query.lower() or "refactor" in request.query.lower():
            return self.agents[AgentType.CODER]
        elif "document" in request.query.lower() or "write" in request.query.lower():
            return self.agents[AgentType.DOCUMENTATION]
        ...
        # For now, prioritize specialized agents over native AI
        for agent_type, agent in capable_agents:
            if agent_type != AgentType.NATIVE_AI:
                return agent
        
        # Fallback to native AI
        return self.agents[AgentType.NATIVE_AI]
    ...
    async def _switch_agent(self, new_agent: BaseAgent):
        ...
        logger.info(f"Switched from {old_agent_type.value} to {new_agent.agent_type.value}")
