...
def eval_once(cases):
    ...
    hits_total = 0
    for case in cases:
        # Support legacy single tag or v1 tags list
        ...
        rows = run_fused_query(
            qs["short"], qs["title"], qs["bm25"], getattr(case, "qvec", None), tag=tag, k=lim["shortlist"], return_components=True
        )
