...
try:
    ...
    # Fallback for when pydantic_evals is not available
    Case = None
    Dataset = None
...
def load_eval_cases_pydantic(profile: str | Path = "gold") -> list[Any]:
    """
    ...
    if p.suffix.lower() == ".jsonl":
        ...
        for obj in data:
            try:
                ...
                if GoldCase is not None:
                    case = GoldCase.model_validate(legacy_obj)
                    cases.append(case)
                else:
                    # Fallback to dict if GoldCase is not available
                    cases.append(legacy_obj)
            except Exception as e:  # pragma: no cover
                warnings.warn(
...
def create_pydantic_evals_dataset(cases: list[Any]) -> Any:
    """
    ...
    """
    if Dataset is None:
        # Fallback when pydantic_evals is not available
        return {"cases": cases}
...
def load_eval_cases(profile: str | Path = "gold") -> list[Any]:
    """
