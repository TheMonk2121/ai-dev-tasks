...
class BacklogParser:
    ...
    def _is_task_line(self, line: str) -> bool:
        ...
        return line.startswith("| B‑") and ("todo" in line or "✅ done" in line or "running" in line)
    ...
    def generate_lanes(
        self, tasks: list[Task], p0_count: int = 5, p1_count: int = 7, p2_count: int = 8
    ) -> dict[str, list[Task]]:
...
def main():
    ...
    parser.add_argument(
        "--generate-sections", action="store_true", help="Print P0/P1/P2 lanes and AI-exec queue markdown"
    )
    parser.add_argument(
        "--write-sections", action="store_true", help="Inject P0/P1/P2 lanes and AI-exec queue into backlog file"
    )
    ...
    if args.filter_status or args.filter_priority:
        filtered = parser.find_tasks_by_criteria(parsed.tasks, status=args.filter_status, priority=args.filter_priority)
