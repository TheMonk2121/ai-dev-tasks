...
class EvaluationSuiteRunner:
    ...
    def __init__(self):
        ...
        self.configs = {
            "ops_smoke": {
                ...
                "dataset": "datasets/eval_cases.jsonl",
                "has_gold": False,
            ...
            "repo_gold": {
                ...
                "dataset": "datasets/dev_gold.jsonl",
                "has_gold": True,
        ...
        if eval_type == "repo_gold":
            ...
            try:
                result = subprocess.run([
                    "python3", "scripts/leakage_guard.py",
                    "--action", "assert-clean",
                    "--eval-file", self.configs[eval_type]["dataset"]
    ...
    def run_gate_validation(self, eval_type: str, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if eval_type == "repo_gold":
            ...
            try:
                cmd = [
                    "python3", "scripts/gate_and_promote.py",
                    "--action", "gate",
                    "--config-hash", f"{eval_type}_{time.strftime('%Y%m%d_%H%M%S')}",
                    "--evaluation-results", str(temp_results_file)
                ]
    ...
    def run_full_suite(self, eval_types: List[str] = None) -> Dict[str, Any]:
        ...
        print("=" * 50)
        print(f"✅ Successful evaluations: {suite_results['summary']['successful']}/{suite_results['summary']['total_evaluations']}")
...
def main():
    ...
    parser.add_argument("--eval-types", nargs="+", choices=["ops_smoke", "repo_gold"], 
                       help="Evaluation types to run (default: both)")
    parser.add_argument("--ops-smoke-only", action="store_true", 
                       help="Run only ops smoke evaluation")
    parser.add_argument("--repo-gold-only", action="store_true", 
                       help="Run only repo-gold evaluation")
