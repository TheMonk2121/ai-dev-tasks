...
class ProductionDeploymentValidator:
    ...
    async def run_all_validations(self) -> Dict[str, Any]:
        ...
        try:
            ...
            results["environment_validation"] = await self._validate_environment()

            # Run configuration validation
            logger.info("Running configuration validation...")
            results["configuration_validation"] = await self._validate_configuration()
            ...
            results["deployment_validation"] = await self._validate_deployment()

            # Generate summary
            total_duration = (time.time() - start_time) * 1000
            results["summary"] = self._generate_validation_summary(total_duration)
    ...
    async def _test_cache_configuration(self) -> ValidationResult:
        ...
        try:
            ...
            pool_info = await self.cache_service.get_connection_pool_info()
            pool_ok = pool_info and pool_info.get("pool_size", 0) > 0
    ...
    async def _test_security_configuration(self) -> ValidationResult:
        ...
        try:
            ...
            pool_info = await self.cache_service.get_connection_pool_info()
            secure_connections = pool_info.get("ssl_enabled", False) if pool_info else False
