...
try:
    ...
    from dspy_rag_system.src.utils.retry_wrapper import retry_with_backoff
    from dspy_rag_system.src.utils.prompt_sanitizer import sanitize_input
except ImportError:
...
class TaskExecutionEngine:
    ...
    def execute_task(self, task_id: str, auto_confirm: bool = False) -> bool:
        ...
        if task.human_required:
            logger.warning(f"Task {task_id} requires human input: {task.human_reason}")
            if not auto_confirm:
                response = input(f"Continue with human-required task {task_id}? (y/N): ")
                if response.lower() != 'y':
        ...
        if not self._check_dependencies(task):
            logger.error(f"Task {task_id} dependencies not met")
            return False
...
Examples:
  ...
  python process_tasks.py validate               # Validate dependencies
    ...
    list_parser.add_argument('--status', choices=['todo', 'pending', 'completed', 'failed'], 
                           help='Filter by status')
    list_parser.add_argument('--priority', choices=['🔥', '📈', '⭐', '🔧'], 
    ...
    execute_parser.add_argument('--auto-confirm', action='store_true',
                              help='Auto-confirm human-required tasks')
    
    # Auto command
    auto_parser = subparsers.add_parser('auto', help='Auto-execute next priority tasks')
    auto_parser.add_argument('--max-tasks', type=int, default=5,
    ...
    # Reset command
    reset_parser = subparsers.add_parser('reset', help='Reset execution state')
    
    # Validate command
    validate_parser = subparsers.add_parser('validate', help='Validate task dependencies')
    ...
    try:
        if args.command == 'list':
            ...
            if args.format == 'json':
                ...
                print("-" * 80)
                for task in tasks:
                    print(f"{task.id:<10} {task.priority.value:<3} {task.points:<6} {task.status.value:<10} {task.title}")
