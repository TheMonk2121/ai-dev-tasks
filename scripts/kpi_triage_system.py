...
class KPITriageSystem:
    ...
    def triage_kpi_breach(self, 
                         metric_name: str,
                         current_value: float,
                         threshold_value: float,
                         breach_type: str = "below") -> Dict[str, Any]:
    ...
    def get_triage_history(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        with open(self.triage_log_file, "r") as f:
            lines = f.readlines()
    ...
    def get_kpi_status(self) -> Dict[str, Any]:
        ...
        kpi_status = {
            "timestamp": datetime.now().isoformat(),
            "kpis": {
                "oracle_prefilter_rate": {
                    "current": 0.87,
                    "threshold": 0.85,
                    "status": "healthy"
                },
                "reader_used_gold_rate": {
                    "current": 0.72,
                    "threshold": 0.70,
                    "status": "healthy"
                },
...
def main():
    ...
    if args.action == "triage":
        ...
        for entry in history:
            status_emoji = "✅" if entry["status"] == "resolved" else "⚠️" if entry["status"] == "requires_manual_intervention" else "🔄"
