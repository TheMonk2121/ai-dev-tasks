...
class LeakageGuard:
    ...
    def __init__(self, few_shot_pool_file: str = "datasets/few_shot_pool.jsonl"):
    ...
    def _load_few_shot_pool(self):
        ...
        if os.path.exists(self.few_shot_pool_file):
            with open(self.few_shot_pool_file, 'r') as f:
                for line in f:
                    if line.strip():
    ...
    def assert_no_leakage(self, eval_file: str):
        ...
        if not validation['is_clean']:
            ...
            raise RuntimeError(f"Data leakage detected: {validation['leakage_count']} records overlap between few-shot and evaluation sets")
        
        print(f"✅ No leakage detected: {validation['total_eval_records']} eval records, {validation['total_few_shot_ids']} few-shot IDs")
...
def main():
    ...
    parser.add_argument("--pool-file", default="datasets/few_shot_pool.jsonl", help="Few-shot pool file")
