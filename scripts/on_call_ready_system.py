...
class OnCallReadySystem:
    ...
    def emergency_rollback(self, 
                          ...
                          reason: str = "Emergency rollback") -> Dict[str, Any]:
        ...
        all_steps_successful = all(
            step["success"] for step in rollback_result["steps"].values()
        )
    ...
    def control_blast_radius(self, 
                           canary_percentage: int = 50,
                           green_eval_passes: int = 2) -> Dict[str, Any]:
        ...
        all_checks_passed = all(
            check["passed"] for check in blast_control["checks"].values()
        )
    ...
    def verify_definition_of_done(self) -> Dict[str, Any]:
        ...
        all_criteria_passed = all(
            criterion["passed"] for criterion in dod_result["criteria"].values()
        )
...
def main():
    ...
    if args.action == "rollback":
        result = on_call_system.emergency_rollback(
            ...
            reason=args.reason or "Emergency rollback"
        )
        print(f"📊 Rollback result: {result['status']}")
    
    elif args.action == "blast-radius":
        result = on_call_system.control_blast_radius(
            canary_percentage=args.canary_limit,
            green_eval_passes=args.green_passes
        )
