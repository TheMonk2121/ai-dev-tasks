...
"""
README Hotspots Analysis

Analyzes validator_report.json to group README violations by top-level directory
and identify cleanup hotspots for targeted README remediation.
"""
...
def generate_cleanup_plan(hotspots: dict[str, list[str]]) -> list[tuple[str, list[str]]]:
    ...
    print("## Cleanup Plan")
    print()
    print("**Batch 3**: Next offender dir (after 400_guides and 500_research)")
    print("**Batch 4**: docs/")
    print("**Batch 5**: remaining stray READMEs")
    print()
    print("Target directories for batches:")

    cleanup_plan = []
    for i, (directory, files) in enumerate(sorted_hotspots[:5]):
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Analyze README violations by directory")
    ...
    print("📊 Analyzing README hotspots...")
    hotspots = analyze_hotspots(report)

    if not hotspots:
        print("OK No README violations found")
        return
