...
def test_bedrock_access() -> bool:
    ...
    try:
        ...
        claude_models = [
            model
            for model in models
            if "claude" in model.get("modelId", "").lower() and "3-5-sonnet" in model.get("modelId", "").lower()
        ]
        ...
        if claude_models:
            ...
            claude_all = [model for model in models if "claude" in model.get("modelId", "").lower()]
