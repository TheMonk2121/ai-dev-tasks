#!/usr/bin/env python3
import os, sys, json, re, subprocess, shlex
...
from dspy_modules.retriever.query_rewrite import build_channel_queries, PHRASE_HINTS
...
def run_reader_cmd(query, context, tag, case_id):
    ...
    proc = subprocess.run(shlex.split(READER_CMD), input=payload.encode("utf-8"),
...
def eval_reader(cases, gold):
    ...
    for case in cases:
        ...
        context, _meta = build_reader_context(rows, case.query, case.tag, compact=bool(int(os.getenv("READER_COMPACT","1"))))
