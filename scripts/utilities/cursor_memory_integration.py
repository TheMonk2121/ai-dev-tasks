...
class CursorMemoryIntegration:
    ...
    def __init__(self, dsn: str | None = None):
        ...
        self.dsn: str = cast(str, resolved)
        ...
        try:
            with psycopg2.connect(self.dsn) as conn:
                with conn.cursor() as cur:
                    ...
                    turns_data = cast(Sequence[tuple[str, str, str, datetime | None, object | None]], cur.fetchall())
                    ...
                    for turn_id, role, content, timestamp, metadata in turns_data:
                        ...
                        if metadata:
                            if isinstance(metadata, (bytes | bytearray)):
                                try:
                                    meta = cast(dict[str, object], json.loads(metadata.decode("utf-8", "ignore")))
                                except Exception:
                                    meta = {}
                            elif isinstance(metadata, str):
                                try:
                                    meta = cast(dict[str, object], json.loads(metadata))
                                except Exception:
                                    meta = {}
        ...
        try:
            with psycopg2.connect(self.dsn) as conn:
                with conn.cursor() as cur:
                    ...
                    title = cast(str, row[0])
                    content = cast(str, row[1])
                    _metadata = row[2]
                    created_at = cast(datetime | None, row[3])
                    last_activity = cast(datetime | None, row[4])
