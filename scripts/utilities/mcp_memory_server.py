...
async def record_chat_history(args: Mapping[str, object]) -> MemoryResponse:
    ...
    try:
        ...
        # Capture user query
        query_turn_id = cursor_integration.capture_user_query(
            user_input, {"source": "auto_record", "project_dir": project_dir, "llm_name": llm_name}
        )

        # Capture AI response
        response_turn_id = cursor_integration.capture_ai_response(
            system_output,
            query_turn_id,
            {
                "source": "auto_record",
