...
async def get_hot_context(args: dict[str, Any]) -> MemoryResponse:
    ...
    try:
        ...
        return MemoryResponse(success=True, data={"hours": hours, "limit": limit, "bundle": data})
    except Exception as e:
        return MemoryResponse(success=False, data={}, error=str(e))

async def search_hot_memory(args: dict[str, Any]) -> MemoryResponse:
    ...
    try:
        ...
        return MemoryResponse(success=True, data={"query": query, "hours": hours, "hits": hits})
    except Exception as e:
        return MemoryResponse(success=False, data={}, error=str(e))

async def pin_hot_item(args: dict[str, Any]) -> MemoryResponse:
    """Pin a specific item into the hot pool for a limited TTL."""
    try:
        item_id = args.get("item_id")
        ttl_hours = int(args.get("ttl_hours", 48))
        if not item_id:
        ...
        return MemoryResponse(success=True, data={"item_id": item_id, "ttl_hours": ttl_hours, "pinned": ok})
    except Exception as e:
        return MemoryResponse(success=False, data={}, error=str(e))
