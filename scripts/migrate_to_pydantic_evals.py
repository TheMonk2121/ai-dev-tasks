...
def load_eval_cases_pydantic(profile: str | Path = "gold") -> list[GoldCase]:
    ...
    # Resolve path (same logic as legacy function)
    ...
    for obj in data:
        try:
            # Handle legacy data that might be missing required fields
            legacy_obj = obj.copy()

            # Set default mode if missing (legacy compatibility)
            if "mode" not in legacy_obj:
                legacy_obj["mode"] = "retrieval"  # Default to retrieval mode
