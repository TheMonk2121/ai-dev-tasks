    ...
    def validate_database_health(self) -> ValidationResult:
        ...
        try:
            ...
            details = {
                ...
                "episodic_logs_count": episodic_count,
    ...
    def validate_monitoring_health(self) -> ValidationResult:
        ...
        try:
            ...
            details = {
                ...
                "monitoring_available": redis_healthy or metrics_healthy
            }
            
            if redis_healthy and metrics_healthy:
                return ValidationResult(
                    component="monitoring",
                    status=True,
                    message="All monitoring systems healthy",
                    details=details,
                    timestamp=datetime.now()
                )
            elif redis_healthy or metrics_healthy:
                return ValidationResult(
                    component="monitoring",
                    status=True,
                    message="Some monitoring systems healthy",
                    details=details,
                    timestamp=datetime.now()
                )
            else:
        ...
        try:
            ...
            if monitoring_url:
                response = requests.post(
                    f"{monitoring_url}/validation-alert",
                    json=report,
                    timeout=10
                )
                if response.status_code == 200:
...
def main():
    ...
    parser.add_argument("--type", choices=["pre", "post", "rollback"], required=True,
                       help="Type of validation to perform")
    parser.add_argument("--output", default="validation_report.json",
                       help="Output file for validation report")
    parser.add_argument("--alert", action="store_true",
                       help="Send validation alert to monitoring system")
