...
class StateManager:
        ...
        try:
            ...
            """,
                (
                    ...
                    json.dumps(
                        {"execution_time": execution_time, "error_message": error_message, "retry_count": retry_count}
                    ),
                ),
            )
    ...
    def update_task_progress(self, task_id: str, progress: float, message: str | None = None) -> bool:
    ...
    def retry_task_execution(self, task_id: str, error_message: str | None = None) -> bool:
        ...
        try:
            ...
            cursor.execute(
                ...
                WHERE task_id = ?
            """,
                (TaskStatus.PENDING.value, retry_count, error_message, datetime.now().isoformat(), task_id),
            )
            ...
            cursor.execute(
                ...
                VALUES (?, ?, ?, ?)
            """,
                (task_id, TaskStatus.PENDING.value, datetime.now().isoformat(), f"Retry attempt {retry_count}"),
            )
...
def main():
    ...
    parser.add_argument(
        "--action",
        choices=["start", "complete", "status", "history", "stats", "reset"],
        default="status",
