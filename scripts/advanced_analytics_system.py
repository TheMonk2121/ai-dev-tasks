...
class AnalyticsDatabase:
    ...
    def get_optimization_opportunities(self, priority: InsightLevel | None = None) -> list[OptimizationOpportunity]:
        ...
        for row in cursor.fetchall():
            opportunity = OptimizationOpportunity(
                ...
                current_value=row[2],
                potential_value=row[3],
                improvement_percentage=row[4],
                effort_required=row[5],
                priority=InsightLevel(row[6]),
                description=row[6],
    ...
    def get_trend_analysis(self) -> list[TrendAnalysis]:
        ...
        for row in cursor.fetchall():
            ...
            data_points_data = json.loads(row[6]) if row[6] else []
            data_points = []
            for dp_data in data_points_data:
                metric = PerformanceMetric(
                    ...
                    timestamp=dp_data.get("timestamp", 0.0),
...
class OptimizationAnalyzer:
    ...
    def _assess_priority(self, improvement_percentage: float, effort_required: str) -> InsightLevel:
        ...
        effort_score = self.config.effort_priority_mapping.get(effort_required, 0.5)
...
def main():
    ...
    print(f"  Opportunities identified: {analysis_result['opportunities_identified']}")
    print(f"  Trends analyzed: {analysis_result['trends_analyzed']}")
    ...
    print(f"  High-priority opportunities: {status['high_priority_opportunities']}")
