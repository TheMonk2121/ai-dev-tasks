...
class OpsRhythmSystem:
    ...
    def run_nightly_smoke(self) -> dict[str, Any]:
        ...
        smoke_tests = {
            ...
            "negatives": {"description": "Negative test cases", "command": "python3 scripts/run_negative_tests.py"},
        }
    ...
    def refresh_few_shots(self) -> dict[str, Any]:
        ...
        # Determine overall status
        all_steps_passed = all(step["success"] for step in refresh_result["steps"].values())
    ...
    def _run_smoke_test(self, test_name: str, test_config: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            result = subprocess.run(
                test_config["command"], shell=True, capture_output=True, text=True, timeout=300  # 5 minute timeout
            )
    ...
    def _generate_sop_cards(self, triage_result: dict[str, Any]) -> dict[str, Any]:
        ...
        return {"sop_cards": sop_cards, "total_cards": len(sop_cards), "timestamp": datetime.now().isoformat()}
    ...
    def _apply_config_nudges(self, triage_result: dict[str, Any]) -> dict[str, Any]:
        ...
        for case in triage_result["worst_cases"]:
            if "RRF weights" in case["recommendation"]:
                nudges.append({"config": "RRF_WEIGHTS", "action": "increase_dense_weight", "value": 0.6})
            elif "reranker" in case["recommendation"]:
                nudges.append({"config": "RERANK_PREWARM", "action": "enable", "value": True})

        return {"nudges": nudges, "total_nudges": len(nudges), "timestamp": datetime.now().isoformat()}
    ...
    def _trigger_smoke_alert(self, smoke_result: dict[str, Any]):
        ...
        failed_tests = [
            test_name for test_name, test_result in smoke_result["tests"].items() if not test_result["passed"]
        ]
    ...
    def _log_rhythm_result(self, result: dict[str, Any]):
        ...
        log_entry = {"timestamp": datetime.now().isoformat(), "type": "rhythm_result", "data": result}
    ...
    def get_rhythm_history(self, limit: int = 10) -> list[dict[str, Any]]:
        ...
        with open(self.rhythm_log_file) as f:
            lines = f.readlines()
