#!/usr/bin/env python3
"""
Bootstrap Repo-Gold Dataset Creator
Samples chunks, proposes gt_answer from sentences, drafts queries, and creates dev_gold.jsonl
"""
...
class RepoGoldDatasetBootstrap:
    ...
    def sample_representative_chunks(self, target_count: int = 40) -> List[Dict[str, Any]]:
                ...
                with conn.cursor() as cur:
                    cur.execute("""
                        ...
                        ORDER BY 
                            ...
                            dc.chunk_index
                        LIMIT %s
                    """, (target_count * 2,))  # Get 2x to allow for filtering
        ...
        print(f"✅ Sampled {len(sampled_chunks)} chunks from {len(set(c.get('file_path', '') for c in sampled_chunks))} files")
    ...
    def draft_query_from_content(self, content: str, filename: str) -> str:
        ...
        for word in words:
            if word in ['run', 'execute', 'create', 'build', 'install', 'configure', 'set', 'use', 'add', 'remove', 'start', 'stop', 'restart']:
                actions.append(word)
            elif word in ['config', 'script', 'command', 'file', 'database', 'index', 'vector', 'embedding', 'evaluation', 'rag', 'dspy']:
    ...
    def categorize_chunk(self, chunk: Dict[str, Any]) -> str:
        ...
        # Ops/Health (6)
        if any(term in content or term in filename for term in ['env', 'environment', 'prefix', 'cache', 'rollback', 'canary']):
            return 'ops_health'
        
        # DB workflows (6)
        if any(term in content or term in filename for term in ['sql', 'database', 'vector', 'fts', 'gin', 'pgvector', 'ann']):
            return 'db_workflows'
...
def main():
    ...
    parser.add_argument("--db", default="postgresql://danieljacobs@localhost:5432/ai_agency", help="Database connection string")
