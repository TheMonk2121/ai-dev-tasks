...
try:
    from dspy_rag_system.src.utils.ltst_memory_system import LTSTMemorySystem
    from dspy_rag_system.src.dspy_modules.rag_system import create_rag_interface
    from dspy_rag_system.src.dspy_modules.vector_store import HybridVectorStore
    ...
    def calculate_latency_metrics(self) -> LatencyMetrics:
        ...
        if not self.latencies:
            return LatencyMetrics(
                ...
                min_ms=0.0, max_ms=0.0, total_requests=0, successful_requests=0, failed_requests=0
            )
    ...
    def calculate_reranker_lift(self) -> RerankerMetrics:
        ...
        if not self.baseline_results or not self.reranked_results:
            return RerankerMetrics(
                baseline_recall=0.0, reranked_recall=0.0, reranker_lift=0.0,
                reranker_latency_ms=0.0, reranker_available=False
            )
...
class BaselineMetricsCollector:
    ...
    async def collect_all_metrics(self) -> BaselineMetrics:
        ...
        baseline_recall = await self.reranker_collector.measure_baseline_performance(self.test_queries)
        reranked_recall = await self.reranker_collector.measure_reranked_performance(self.test_queries)
    ...
    def _calculate_overall_score(self, latency: LatencyMetrics, reranker: RerankerMetrics, health: HealthMetrics) -> float:
    ...
    def save_metrics(self, metrics: BaselineMetrics, output_file: str = None):
