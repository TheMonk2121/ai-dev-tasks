...
class GoldCaseHypothesisAnalysis:
    ...
    def _analyze_schema_validation(self) -> dict[str, Any]:
        ...
        for case in self.cases:
            ...
            if not case.id:
                issues["missing_required_fields"].append(f"Case {case.id}: Missing ID")
            if not case.query:
                issues["empty_queries"].append(f"Case {case.id}: Empty query")
            if not case.tags:
                issues["empty_tags"].append(f"Case {case.id}: Empty tags")

            # Check mode requirements
            if case.mode == Mode.reader and not case.gt_answer:
                issues["mode_requirement_violations"].append(f"Case {case.id}: Reader mode missing gt_answer")
            elif case.mode == Mode.retrieval and not case.expected_files and not case.globs:
                issues["mode_requirement_violations"].append(
        ...
        print(f"Empty queries: {len(issues['empty_queries'])}")
        print(f"Empty tags: {len(issues['empty_tags'])}")
        print(f"Mode requirement violations: {len(issues['mode_requirement_violations'])}")

        if issues["mode_requirement_violations"]:
            print("\nMode requirement violations:")
            for violation in issues["mode_requirement_violations"][:5]:
                print(f"  ❌ {violation}")
    ...
    def _analyze_query_quality(self) -> dict[str, Any]:
        ...
        for case in self.cases:
            ...
            # Check query length
            if len(query) < 10:
                issues["too_short"].append(f"Case {case.id}: '{query}' (too short)")
            elif len(query) > 200:
                issues["too_long"].append(f"Case {case.id}: '{query}' (too long)")

            # Check for unclear questions
            if not query.endswith("?"):
                issues["unclear_questions"].append(f"Case {case.id}: '{query}' (not a question)")

            # Check for file-specific questions
            if any(pattern in query.lower() for pattern in [".md", ".py", ".json", "according to"]):
                issues["file_specific_questions"].append(f"Case {case.id}: '{query}'")
            ...
            if any(re.match(pattern, query.lower()) for pattern in generic_patterns):
                issues["generic_questions"].append(f"Case {case.id}: '{query}'")
        ...
        print(f"Generic questions: {len(issues['generic_questions'])}")
    ...
    def _analyze_answer_quality(self) -> dict[str, Any]:
        ...
        for case in reader_cases:
            if not case.gt_answer:
                issues["missing_answers"].append(f"Case {case.id}: No ground truth answer")
                continue
            ...
            # Check answer length
            if len(answer) < 20:
                issues["too_short_answers"].append(f"Case {case.id}: '{answer[:50]}...' (too short)")
            elif len(answer) > 500:
                issues["too_long_answers"].append(f"Case {case.id}: '{answer[:50]}...' (too long)")

            # Check for incomplete answers
            if answer.endswith("...") or "TODO" in answer or "FIXME" in answer:
                issues["incomplete_answers"].append(f"Case {case.id}: '{answer[:50]}...' (incomplete)")

            # Check for unclear answers
            if answer.lower() in ["not in context.", "not found.", "unknown.", "n/a"]:
                issues["unclear_answers"].append(f"Case {case.id}: '{answer}' (unclear)")

        avg_answer_length = sum(answer_lengths) / len(answer_lengths) if answer_lengths else 0

        print(f"Reader mode cases: {len(reader_cases)}")
        print(f"Average answer length: {avg_answer_length:.1f} characters")
        print(f"Missing answers: {len(issues['missing_answers'])}")
        print(f"Too short answers: {len(issues['too_short_answers'])}")
        print(f"Too long answers: {len(issues['too_long_answers'])}")
        print(f"Incomplete answers: {len(issues['incomplete_answers'])}")
        print(f"Unclear answers: {len(issues['unclear_answers'])}")
    ...
    def _analyze_coverage(self) -> dict[str, Any]:
        ...
        for case in self.cases:
            if case.expected_files:
                for file_path in case.expected_files:
                    ...
                    if "/" in file_path:
                        coverage["directory_coverage"].add(file_path.split("/")[0])
        ...
        print(f"Tag distribution: {dict(coverage['tag_coverage'].most_common(10))}")
    ...
    def _analyze_duplicates(self) -> dict[str, Any]:
        ...
        for query, count in query_counts.items():
            if count > 1:
                duplicates["duplicate_queries"].append(f"'{query}' appears {count} times")
        ...
        for answer, count in answer_counts.items():
            if count > 1:
                duplicates["duplicate_answers"].append(f"Answer appears {count} times")
        ...
        for case in self.cases:
            if case.mode == Mode.retrieval:
                if not case.expected_files and not case.globs:
                    issues["retrieval_without_files"].append(f"Case {case.id}: No expected files or globs")
            elif case.mode == Mode.reader:
                if not case.gt_answer:
                    issues["reader_without_answers"].append(f"Case {case.id}: No ground truth answer")
            elif case.mode == Mode.decision:
                if not case.expected_decisions:
                    issues["decision_without_decisions"].append(f"Case {case.id}: No expected decisions")
    ...
    def _analyze_hypothesis_violations(self) -> dict[str, Any]:
        ...
        for case in self.cases:
            if not case.id or not case.query or not case.tags:
                violations["invalid_test_cases"].append(f"Case {case.id}: Missing required fields")

            # Check for poor quality queries
            if len(case.query) < 10 or not case.query.endswith("?"):
                violations["poor_quality_queries"].append(f"Case {case.id}: Poor quality query")

            # Check for missing supervision
            if case.mode == Mode.reader and not case.gt_answer:
                violations["missing_supervision"].append(f"Case {case.id}: Reader mode without answer")
            elif case.mode == Mode.retrieval and not case.expected_files and not case.globs:
                violations["missing_supervision"].append(f"Case {case.id}: Retrieval mode without files")
            elif case.mode == Mode.decision and not case.expected_decisions:
                violations["missing_supervision"].append(f"Case {case.id}: Decision mode without decisions")
    ...
    def generate_report(self) -> str:
        ...
        report.append("")
        total_cases = self.analysis_results["basic_stats"]["total_cases"]
        schema_issues = len(self.analysis_results["schema_validation"]["mode_requirement_violations"])
        file_issues = len(self.analysis_results["file_existence"]["missing_files"])
        query_issues = len(self.analysis_results["query_quality"]["issues"]["too_short"]) + len(
            self.analysis_results["query_quality"]["issues"]["unclear_questions"]
        )
...
def main():
    ...
    # Print summary
    print("\n🎯 SUMMARY:")
    print(f"Total cases analyzed: {results['basic_stats']['total_cases']}")
    print(f"Schema violations: {len(results['schema_validation']['mode_requirement_violations'])}")
    print(f"Missing files: {len(results['file_existence']['missing_files'])}")
    print(
        f"Query quality issues: {len(results['query_quality']['issues']['too_short']) + len(results['query_quality']['issues']['unclear_questions'])}"
    )
