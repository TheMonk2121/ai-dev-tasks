...
def validate_gold_dataset() -> List[str]:
    ...
    gold_file = Path("evals/gold/v1/gold_cases.jsonl")
    manifest_file = Path("evals/gold/v1/manifest.json")

    # Check files exist
    if not gold_file.exists():
        issues.append("Gold dataset file missing: evals/gold/v1/gold_cases.jsonl")
        return issues

    if not manifest_file.exists():
        issues.append("Manifest file missing: evals/gold/v1/manifest.json")
        return issues
