...
class LocalLLMIntegration:
    ...
    def __init__(
        ...
        if use_bedrock:
            if not BEDROCK_AVAILABLE or BedrockClient is None:
    ...
    def _call_bedrock_llm(self, prompt: str, max_tokens: int = 1000) -> str:
        ...
        if not self.bedrock_client:
            print("⚠️ Bedrock client not available, falling back to local LLM")
            self.use_bedrock = False
            return self._call_ollama_llm(prompt, max_tokens)
...
class OfficialRAGCheckerEvaluator:
    ...
    def create_official_test_cases(self) -> List[RAGCheckerInput]:
        ...
        max_cases = int(os.getenv("RAGCHECKER_MAX_TEST_CASES", "1"))
        if max_cases < len(test_cases):
            print(f"🧪 Limited test mode: Using {max_cases} test case(s) (reduced from {len(test_cases)})")
    ...
    def run_local_llm_evaluation(self, input_data: List[Dict[str, Any]], api_base: str) -> Dict[str, Any]:
        ...
        for i, case in enumerate(input_data):
            ...
            if len(gt_words) > 0:
                ...
                # Use semantic similarity if available, fallback to word overlap
                if (
                    hasattr(self, "local_llm")
                    and self.local_llm
                    and hasattr(self.local_llm, "calculate_semantic_query_relevance")
                ):
                    query_relevance = self.local_llm.calculate_semantic_query_relevance(case["query"], case["response"])
                    print(f"🎯 Semantic query relevance: {query_relevance:.3f}")
                else:
                    query_relevance = (
                        len(response_words.intersection(query_words)) / len(query_words) if len(query_words) > 0 else 0
                    )
                query_boost = 0.1 * query_relevance  # Small boost for query relevance
    ...
    def create_fallback_evaluation(self, input_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for i, case in enumerate(input_data):
            ...
            if len(gt_words) > 0:
                ...
                query_boost = 0.05 * query_relevance  # Smaller boost for fallback
    ...
    def run_official_evaluation(
        ...
        print("🧠 Official RAGChecker Single Test Evaluation")
        print("=" * 60)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Official RAGChecker Single Test with Hybrid LLM Support")
    ...
    if not use_bedrock and not use_local_llm:
        ...
        if BEDROCK_AVAILABLE and BedrockClient is not None:
            try:
