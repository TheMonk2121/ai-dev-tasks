...
STABLE_ENV_FILE="${RAGCHECKER_ENV_FILE:-evals/configs/stable_bedrock.env}"
if [ -f "$STABLE_ENV_FILE" ]; then
  echo "📁 Loading stable config: $STABLE_ENV_FILE"
  # shellcheck source=evals/configs/stable_bedrock.env
  # shellcheck disable=SC1091
  source "$STABLE_ENV_FILE"
  echo "🔒 Loaded env from $STABLE_ENV_FILE … lock=True"
else
  echo "❌ Stable config not found: $STABLE_ENV_FILE"
  echo "💡 Run: cp evals/configs/stable_bedrock.env.template evals/configs/stable_bedrock.env"
  exit 1
fi
