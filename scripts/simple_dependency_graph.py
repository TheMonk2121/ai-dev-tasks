#!/usr/bin/env python3
"""
Simple Dependency Graph Builder for Vector-Based System Mapping
...
class SimpleDependencyGraphBuilder:
    ...
    def export_graph(self, output_dir: str = "metrics/graphs") -> Dict[str, str]:
        ...
        png_file = os.path.join(output_dir, "simple_dependency_graph.png")
        try:
            plt.figure(figsize=(16, 12))
            pos = nx.spring_layout(self.graph, k=1, iterations=30)
            nx.draw(
                self.graph,
                pos,
                with_labels=False,
            ...
            plt.title("Python Dependency Graph (Simplified)")
            plt.tight_layout()
            plt.savefig(png_file, dpi=120, bbox_inches="tight")
            plt.close()
            export_files["png"] = png_file
            print(f"📊 Graph exported to PNG: {png_file}")
        except Exception as e:
