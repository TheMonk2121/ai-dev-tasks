...
from src.utils.retry_wrapper import retry_llm
...
def _load_fast_path_config() -> dict[str, Any]:
    """Load fast-path configuration from pydantic-settings"""
    try:
        from ..config import get_settings

        settings = get_settings()
        return {
            "enabled": settings.rag.fast_path_enabled,
            "max_length": settings.rag.max_query_len,
...
class MistralLLM(dspy.Module):
    ...
    def __init__(
        ...
        settings = get_settings()
        self.timeout = timeout or settings.performance.llm_request_timeout
        ...
        # Set default timeout for all requests - using requests.Session.timeout property
        # Note: requests.Session doesn't have a timeout attribute, we need to pass it to individual requests
        self._session = sess
