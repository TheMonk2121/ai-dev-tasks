...
class Entity:
    ...
    confidence: float
    context: str
    mentions: list[int] = field(default_factory=list)  # Turn indices where entity appears
        ...
        for entity_text in fact.entities:
            if entity_text not in entity_texts:
                ...
                entity = Entity(
                    ...
                    context=context,
                    mentions=mentions,
                    aliases=_find_entity_aliases(entity_text),
                )
...
def link_entities(facts: list[Fact], entities: list[Entity]) -> list[EntityLink]:
    ...
    try:
            ...
            for i, entity1 in enumerate(fact_entities):
                for j, entity2 in enumerate(fact_entities[i + 1 :], i + 1):
                    ...
                    if entity1_obj and entity2_obj:
                        ...
                        rel_type = _determine_relationship_type(fact)
        ...
        links = _fallback_entity_linking(facts)
...
def _find_entity_aliases(entity_text: str) -> list[str]:
...
def _determine_relationship_type(fact: Fact) -> str:
...
def _fallback_entity_linking(facts: list[Fact]) -> list[EntityLink]:
