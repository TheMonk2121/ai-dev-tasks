...
<!-- est_hours: 20 -->
<!-- acceptance: System includes constitution-aware Pydantic models, 95%+ role output validation, 50% runtime error reduction, constitution invariants enforced via type system, typed debug logs with context schema, constitution-aware validation integrated with existing Pydantic infrastructure, and async Pydantic validation (STMContext, EpisodeRecord, EpisodeEvent) for 50-60% I/O performance improvement with zero new dependencies -->
