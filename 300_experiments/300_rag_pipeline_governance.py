...
class RAGPipelineGovernance:
    ...
    def create_pipeline_graph(self, pipeline_config: Dict[str, Any], pipeline_id: str) -> str:
        ...
        for i, stage in enumerate(stages):
            ...
            stage_config = pipeline_config.get(stage.value, {})
            ...
            node_data["node_type"] = stage_node.node_type.value
            node_data["stage"] = stage_node.stage.value
            graph.add_node(stage_node_id, **node_data)

            # Add parameter nodes for this stage
            for param_name, param_value in stage_config.get("parameters", {}).items():
                ...
                param_node = PipelineNode(
                    ...
                    metadata={"parent_stage": stage_node_id, "parameter_name": param_name, "pipeline_id": pipeline_id},
                )

                # Convert enum to string for storage
                param_data = param_node.__dict__.copy()
                param_data["node_type"] = param_node.node_type.value
                param_data["stage"] = None  # Parameters don't have stages
        ...
        self.pipeline_metadata[pipeline_id] = {
            ...
            "created_at": pipeline_config.get("timestamp"),
    ...
    def _apply_syntactic_pipeline_augmentation(self, original_graph: nx.DiGraph, augmented_id: str) -> str:
        ...
        for i in range(len(stage_nodes) - 1):
            ...
            if current_stage not in [PipelineStage.INGEST, PipelineStage.VALIDATE] and next_stage not in [
                PipelineStage.INGEST,
                PipelineStage.VALIDATE,
            ]:
    ...
    def validate_pipeline(self, pipeline_id: str) -> Dict[str, Any]:
        ...
        validation_results = {"valid": True, "warnings": [], "errors": [], "suggestions": []}
        ...
        for required_stage in required_stages:
            if required_stage not in present_stages:
                validation_results["errors"].append(f"Missing required stage: {required_stage}")
                validation_results["valid"] = False

        # Check for unusual patterns
        if self._detect_unusual_patterns(graph):
            validation_results["warnings"].append("Pipeline contains unusual patterns")
            validation_results["suggestions"].append("Consider using a known-good variant")

        # Check parameter ranges
        param_issues = self._check_parameter_ranges(graph)
        validation_results["warnings"].extend(param_issues)
    ...
    def auto_fill_missing_steps(self, partial_pipeline_id: str) -> str:
        ...
        for stage in missing_stages:
            ...
            stage_node = PipelineNode(
                ...
                metadata={"stage_order": len(present_stages), "auto_filled": True, "pipeline_id": partial_pipeline_id},
            )
