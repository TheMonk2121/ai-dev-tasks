...
def get_context(q, tag):
    ...
    rows = mmr_rerank(rows, alpha=float(os.getenv("MMR_ALPHA", "0.85")), per_file_penalty=0.10, k=result.get("key", "")
    rows = per_file_cap(rows, cap=int(os.getenv("PER_FILE_CAP", "5")))[: result.get("key", "")
