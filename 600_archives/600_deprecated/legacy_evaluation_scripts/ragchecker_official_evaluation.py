...
def main(argv: list[str] | None = None) -> int:
    ...
    if "--help" in argv or "-h" in argv:
        print(
            "Usage: scripts/ragchecker_official_evaluation.py [--profile gold|mock] [args]"
        )
        print("")
        print("Profiles:")
    ...
    if config_logger_available:
        try:
            ...
            print(f"🔧 Configuration captured: {config_data['run_id']}")
    ...
    if "--outdir" in argv:
        ...
        try:
            ...
            results = evaluator.run_evaluation(
                gold_file="300_evals/data/gold/v1/gold_cases.jsonl",
                limit=5,  # Small test
            )
        except Exception as e:
        ...
        # Add config data to results if available
        if config_logger and config_data:
            results["config"] = config_data
            # Also save config separately
            config_path = Path(outdir) / f"config_{ts}.json"
            config_logger.log_to_file(config_path)
    ...
    _ = ssot_run(
        suite=suite, pass_id=pass_id, out=None, seed=None, concurrency=concurrency
    )
    return 0
    ...
    def _ensure_impl(self) -> None:
        ...
        try:
            ...
            if not impl_path.exists():
                impl_path = Path(
                    "600_archives/600_deprecated/_ragchecker_eval_impl.py"
                ).resolve()
            module = (
                __import__("importlib.machinery", fromlist=["SourceFileLoader"])
            ...
            raise RuntimeError(
                f"Failed to load OfficialRAGCheckerEvaluator implementation: {e}"
            )
