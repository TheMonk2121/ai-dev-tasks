...
def run_evaluation_pass(
    pass_name: str, config: dict[str, Any], output_file: str
) -> dict[str, Any]:
    ...
    try:
        ...
        pass_output_dir = (
            Path("metrics/production_evaluations")
            / f"pass_{pass_name.lower().replace(' ', '_')}"
        )
    ...
    for result in results:
        if result.get("return_code") == 0:
            ...
            analysis["recommendations"].append(
                f"Fix {result['pass_name']} before proceeding"
            )
...
def main():
    ...
    for i, pass_config in enumerate(passes, 1):
        output_file = (
            results_dir
            / f"pass_{i}_{pass_config['name'].lower().replace(' ', '_')}.json"
        )
        ...
        result = run_evaluation_pass(
            pass_config["name"], pass_config["config"], str(output_file)
        )
