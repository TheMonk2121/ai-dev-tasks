...
def load_governance_config(
    config_path: str = "../config/rag_pipeline_governance.json",
) -> dict[str, Any]:
...
def run_governed_evaluation(
    ...
    if validation_results["unusual_patterns"]:
        ...
        if "suggested_variant" in validation_results:
            logger.info(
                f"💡 Suggested variant: {validation_results['suggested_variant']}"
            )
    ...
    test_queries = config.get("evaluation_config", {}).get(
        ...
        [
            ...
            "What are the DSPy integration patterns?",
        ],
    )

    # Evaluate pipeline performance
    evaluation_results = governance.evaluate_pipeline_performance(
        pipeline_config, test_queries
    )

    # Check performance against targets
    performance_targets = config.get("evaluation_config", {}).get(
        "performance_targets", {}
    )
    ...
    if "average_metrics" in evaluation_results:
        ...
        for metric, target in performance_targets.items():
            ...
            logger.info(
                f"📊 {metric}: {actual:.3f} (target: {target:.3f}) {'✅' if actual >= target else '❌'}"
            )
...
def generate_pipeline_variants(
    ...
    for i, variant in enumerate(variants):
        ...
        # Evaluate variant
        results = governance.evaluate_pipeline_performance(
            variant["config"], test_queries
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run RAGChecker evaluation with pipeline governance"
    )
    parser.add_argument(
        "--config",
        default="../config/rag_pipeline_governance.json",
        help="Governance configuration file",
    )
    parser.add_argument("--pipeline-config", help="Custom pipeline configuration file")
    parser.add_argument(
        "--generate-variants", type=int, default=0, help="Generate N pipeline variants"
    )
    ...
    if args.pipeline_config:
        try:
            ...
            logger.error(
                f"Pipeline configuration file not found: {args.pipeline_config}"
            )
            return 1
    ...
    if args.generate_variants > 0:
        ...
        variant_results = generate_pipeline_variants(
            governance, base_config, args.generate_variants
        )
    ...
    if (
        ...
        and "average_metrics" in results["evaluation_results"]
    ):
        ...
        logger.info(
            f"  - Success Rate: {results['evaluation_results'].get('success_rate', 0):.3f}"
        )
