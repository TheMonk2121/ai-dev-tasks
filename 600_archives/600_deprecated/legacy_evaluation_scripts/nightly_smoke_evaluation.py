...
class NightlySmokeEvaluator:
    ...
    def __init__(self, output_dir: str = "metrics/nightly_smoke"):
        ...
        self.smoke_tests: dict[str, CategoryConfig] = {
            "ops_health": {
    ...
    def _run_category_tests(self, category: str, tests: list[str]) -> CategoryResults:
    ...
    def run_category_tests(self, category: str, tests: list[str]) -> CategoryResults:
        """Public wrapper to run tests for a specific category."""
        return self._run_category_tests(category, tests)
    ...
    def _test_database_connectivity(self) -> dict[str, Any]:
        ...
        try:
            try:
                ...
                return {
                    "status": "pass",
                    "message": f"Database DSN configured: {dsn[:20]}...",
                }
            except ImportError:
                ...
                return {
                    "status": "fail",
                    "error": f"Database DSN resolution failed: {e}",
                }
        except Exception as e:
    ...
    def _test_model_availability(self) -> dict[str, Any]:
        ...
        try:
            ...
            if os.getenv("RERANK_ENABLE", "1") == "1":
                ...
                _ = reranker.predict([("query", "document")])
    ...
    def _test_configuration_validation(self) -> dict[str, Any]:
        ...
        missing_configs = [
            config for config in required_configs if not os.getenv(config)
        ]
    ...
    def _test_resource_availability(self) -> dict[str, Any]:
        ...
        try:
            ...
            if free_gb < 2:
                return {
                    "status": "warn",
                    "warning": f"Low disk space: {free_gb}GB available",
                }
            else:
                return {
                    "status": "pass",
    ...
    def _test_evaluation_pipeline(self) -> dict[str, Any]:
        ...
        try:
            ...
            eval_cases_file = os.getenv(
                "EVAL_CASES_FILE", "300_evals/datasets/eval_cases.jsonl"
            )
        ...
        for category, result in self.results.items():
            if result["status"] == "fail":
                recommendations.append(
                    f"Fix {category} failures: {', '.join(result['errors'])}"
                )
            elif result["warnings"]:
                recommendations.append(
                    f"Address {category} warnings: {', '.join(result['warnings'])}"
                )
    ...
    def _print_evaluation_summary(self, evaluation_summary: dict[str, Any]):
        ...
        if evaluation_summary["regressions"]:
            ...
            for regression in evaluation_summary["regressions"]:
                ...
                print(
                    f"  {severity_emoji} {regression['category']}.{regression['test']}: {regression['description']}"
                )
...
def main():
    ...
    if args.category:
        ...
        if args.category in evaluator.smoke_tests:
            ...
            result = evaluator.run_category_tests(args.category, config["tests"])
