...
class RAGCheckerConstitutionValidator(BaseModel):
    ...
    enabled: bool = Field(
        default=True, description="Whether constitution validation is enabled"
    )
    validation_rules: list[Any] = Field(
        default_factory=list, description="Active validation rules"
    )
        ...
        try:
            ...
            if ConstitutionCompliance is not None:
                ...
                validation_results["compliance"] = ConstitutionCompliance(
                    ...
                    violations=[
                        error["message"] for error in validation_results["errors"]
                    ],
                    recommendations=[
    ...
    def validate_ragchecker_metrics(
    ...
    def _validate_input_quality(self, input_data: dict[str, Any]) -> dict[str, Any]:
        ...
        if "retrieved_context" in input_data:
            if (
                not isinstance(input_data["retrieved_context"], list)
                or len(input_data["retrieved_context"]) == 0
            ):
                validation["valid"] = False
                validation["errors"].append(
                    {
    ...
    def _validate_metrics_coherence(
        self, metrics_data: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    def _validate_result_consistency(
        ...
        if "custom_score" in result_data and "ragchecker_overall" in result_data:
            ...
            if isinstance(custom_score, int | float) and isinstance(
                ragchecker_score, int | float
            ):
    ...
    def map_validation_error_to_taxonomy(
        ...
        try:
            ...
            if "validation" in error_type.lower():
                taxonomy_error = (
                    ConstitutionErrorMapper.map_constitution_failure_to_error(
                        "validation_failure",
                        message,
    ...
    def enhance_validation_with_taxonomy(
        ...
        if enhanced_results.get("input_validation") and enhanced_results[
            "input_validation"
        ].get("errors"):
        ...
        if enhanced_results.get("metrics_validation") and enhanced_results[
            "metrics_validation"
        ].get("errors"):
        ...
        if enhanced_results.get("result_validation") and enhanced_results[
            "result_validation"
        ].get("errors"):
        ...
        enhanced_results["taxonomy_summary"] = {
            "total_categorized_errors": sum(
                [
                    ...
                    len(
                        enhanced_results.get("metrics_validation", {}).get("errors", [])
                    ),
                    len(
                        enhanced_results.get("result_validation", {}).get("errors", [])
                    ),
                ]
            ),
            "categorization_success_rate": self._calculate_categorization_success_rate(
                enhanced_results
            ),
    ...
    def _calculate_categorization_success_rate(
        self, validation_results: dict[str, Any]
    ) -> float:
        ...
        for validation_type in [
            ...
            if validation_results.get(validation_type) and validation_results[
                validation_type
            ].get("errors"):
    ...
    def _get_error_type_distribution(
        self, validation_results: dict[str, Any]
    ) -> dict[str, int]:
