...
class PrecisionClimbV2Evaluator:
    ...
    def enhanced_evidence_filter(
        ...
        if (
            ...
            and self.enhanced_filter
            and self.cross_encoder
        ):
            print("🔄 Using cross-encoder enhanced filtering")
            result = self.enhanced_filter.filter_with_cross_encoder(
    ...
    def calculate_claim_confidence(
        self, claim: str, contexts: list[str], top_k: int = 2
    ) -> float:
        ...
        anchor_component = (
            signals["jaccard"] * weights[1]
        )  # Using jaccard as anchor proxy
    ...
    def run_staged_evaluation(
        ...
        for layer in layers:
            ...
            layer_results = self.run_precision_climb_evaluation(
                layer, enable_cross_encoder
            )
            ...
            if precision >= 0.20:
                print(
                    f"🎯 Target precision (≥0.20) achieved with {layer}: {precision:.3f}"
                )
                break
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="RAGChecker Precision-Climb v2 Evaluation"
    )
    parser.add_argument(
        "--layer",
    ...
    parser.add_argument(
        "--staged",
        action="store_true",
        help="Run staged evaluation across multiple layers",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="precision_climb_v2_results.json",
    ...
    if args.staged:
        results = evaluator.run_staged_evaluation(
            enable_cross_encoder=args.enable_cross_encoder
        )
    else:
        results = evaluator.run_precision_climb_evaluation(
            args.layer, args.enable_cross_encoder
        )
