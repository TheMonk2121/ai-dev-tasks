...
class HealthGatedEvaluator:
    ...
    def _check_index_presence(self):
        ...
        if not os.path.exists(dspy_rag_path):
            self.failed_checks.append(
                f"DSPy RAG system path not found: {dspy_rag_path}"
            )
            return
    ...
    def _check_token_budget(self):
        ...
        if max_tokens > 4096:
            self.warning_checks.append(
                f"High token limit: {max_tokens} (may cause performance issues)"
            )
        ...
        if context_max_chars > 8000:
            self.warning_checks.append(
                f"High context limit: {context_max_chars} characters"
            )
    ...
    def _check_prefix_leakage(self):
        ...
        if bm25_text_field == embedding_text_field:
            self.warning_checks.append(
                "BM25 and embedding text fields are the same (potential leakage)"
            )
    ...
    def _check_model_availability(self):
        ...
        try:
            ...
            self.failed_checks.append(
                f"Embedding model not available: {embedding_model} - {e}"
            )
        ...
        if os.getenv("RERANK_ENABLE", "1") == "1":
            ...
            try:
                ...
                self.failed_checks.append(
                    f"Rerank model not available: {rerank_model} - {e}"
                )
    ...
    def _check_config_validation(self):
        ...
        if topk_vec > 500:
            self.warning_checks.append(
                f"High vector top-k: {topk_vec} (may cause performance issues)"
            )

        if topk_bm25 > 500:
            self.warning_checks.append(
                f"High BM25 top-k: {topk_bm25} (may cause performance issues)"
            )
        ...
        if max_in_flight > 3:
            self.warning_checks.append(
                f"High concurrency: {max_in_flight} (may cause rate limiting)"
            )
    ...
    def print_health_report(
        self, is_healthy: bool, failed_checks: list[str], warning_checks: list[str]
    ):
...
def main():
    ...
    parser.add_argument(
        "--check-only",
        action="store_true",
        help="Only run health checks, don't proceed with evaluation",
    )
    parser.add_argument(
        "--disable-check", action="append", help="Disable specific health check"
    )
