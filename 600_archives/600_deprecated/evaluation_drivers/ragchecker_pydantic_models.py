...
class RAGCheckerInput(BaseModel):
    ...
    query: str = Field(
        description="The user query to be evaluated", min_length=1, max_length=10000
    )

    gt_answer: str = Field(
        description="Ground truth answer for evaluation", min_length=1, max_length=50000
    )

    response: str = Field(
        description="Generated response to be evaluated", min_length=1, max_length=50000
    )

    retrieved_context: list[str] = Field(
        description="List of retrieved context strings for evaluation"
    )
    ...
    def validate_query_id(cls, v: str) -> str:
        ...
        if not re.match(r"^[a-zA-Z0-9_-]+$", v):
            raise ValueError(
                "Query ID must contain only alphanumeric characters, underscores, and hyphens"
            )
        return v
...
class RAGCheckerMetrics(BaseModel):
    ...
    context_precision: float = Field(
        description="Context precision score", ge=0.0, le=1.0
    )

    # Generator Metrics
    context_utilization: float = Field(
        description="Context utilization score", ge=0.0, le=1.0
    )
    noise_sensitivity: float = Field(
        description="Noise sensitivity score", ge=0.0, le=1.0
    )
    hallucination: float = Field(
        description="Hallucination score (lower is better)", ge=0.0, le=1.0
    )
...
class RAGCheckerResult(BaseModel):
    ...
    test_case_name: str = Field(
        description="Name of the test case", min_length=1, max_length=200
    )

    query: str = Field(
        description="The query that was evaluated", min_length=1, max_length=10000
    )
    ...
    ragchecker_overall: float = Field(
        description="Overall RAGChecker score", ge=0.0, le=1.0
    )

    comparison: dict[str, Any] = Field(
        description="Comparison data between custom and RAGChecker evaluation",
        default_factory=dict,
    )

    recommendation: str = Field(
        description="Recommendation based on evaluation results",
        min_length=1,
        max_length=1000,
    )
    ...
    def validate_scores(cls, v: dict[str, float]) -> dict[str, float]:
        ...
        for metric, score in v.items():
            if score < 0.0 or score > 1.0:
                raise ValueError(
                    f"Score for metric '{metric}' must be between 0.0 and 1.0, got {score}"
                )
        return v
    ...
    def validate_overall_score(cls, v: float, info: ValidationInfo) -> float:
        ...
        if scores:
            ...
            if abs(v - avg_score) > 0.1:  # Allow some difference for weighted averages
                ...
                print(
                    f"Warning: Overall score {v} differs significantly from average {avg_score}"
                )
        return v
