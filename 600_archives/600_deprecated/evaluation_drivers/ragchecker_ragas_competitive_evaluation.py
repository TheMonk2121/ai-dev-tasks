...
class RAGASCompetitiveEvaluator(EnhancedRAGCheckerWithLimitFeatures):
    ...
    def evaluate_ragas_competitive(self, test_cases: list[Any]) -> dict[str, Any]:
        ...
        evaluation_report = self._generate_ragas_report(
            results, overall_metrics, gate_results
        )
    ...
    def _evaluate_case_ragas_competitive(self, test_case: Any) -> dict[str, Any]:
        ...
        try:
            ...
            ragchecker_result = self.create_fallback_evaluation_with_limit_features(
                eval_data
            )
        ...
        unsupported_claims = self._calculate_unsupported_claims(
            response, test_case.gt_answer
        )
    ...
    def _generate_ragas_report(
        ...
        return {
            ...
            "summary": {
                "precision_target_met": overall_metrics.get("precision", 0)
                >= self.ragas_targets["precision"],
                "recall_target_met": overall_metrics.get("recall_at_20", 0)
                >= self.ragas_targets["recall_at_20"],
                "f1_target_met": overall_metrics.get("f1_score", 0)
                >= self.ragas_targets["f1_score"],
                "faithfulness_target_met": overall_metrics.get("faithfulness", 0)
                >= self.ragas_targets["faithfulness"],
                "unsupported_claims_target_met": overall_metrics.get(
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="RAGAS-Competitive RAGChecker Evaluation"
    )
    parser.add_argument(
        "--output",
        type=str,
        default="ragas_competitive_evaluation.json",
        help="Output file for results",
    )
    parser.add_argument(
        "--test-cases", type=int, default=15, help="Number of test cases to evaluate"
    )
    ...
    print(
        f"   Precision: {overall['precision']:.3f} (target: ≥{evaluator.ragas_targets['precision']})"
    )
    print(
        f"   Recall@20: {overall['recall_at_20']:.3f} (target: ≥{evaluator.ragas_targets['recall_at_20']})"
    )
    print(
        f"   F1 Score: {overall['f1_score']:.3f} (target: ≥{evaluator.ragas_targets['f1_score']})"
    )
    print(
        f"   Faithfulness: {overall['faithfulness']:.3f} (target: ≥{evaluator.ragas_targets['faithfulness']})"
    )
    print(
    ...
    print(
        f"   Faithfulness Target: {'✅' if summary['faithfulness_target_met'] else '❌'}"
    )
    print(
        f"   Unsupported Claims Target: {'✅' if summary['unsupported_claims_target_met'] else '❌'}"
    )
