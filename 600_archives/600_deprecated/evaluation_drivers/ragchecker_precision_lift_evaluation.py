...
class PrecisionLiftEvaluator(OfficialRAGCheckerEvaluator):
    ...
    def evaluate_with_precision_lift(self, test_cases: list[Any]) -> dict[str, Any]:
        ...
        for i, test_case in enumerate(test_cases, 1):
            ...
            case_result, precision_metrics = self._evaluate_case_with_precision_lift(
                test_case
            )
        ...
        evaluation_report = self._generate_precision_lift_report(
            results, precision_metrics_summary
        )

        return evaluation_report

    def _evaluate_case_with_precision_lift(
        self, test_case: Any
    ...
    def _generate_precision_lift_report(
        ...
        precision_metrics_summary: list[dict[str, Any]],
    ) -> dict[str, Any]:
        ...
        precision_stats = self._calculate_precision_lift_statistics(
            precision_metrics_summary
        )
    ...
    def _calculate_precision_lift_statistics(
        self, precision_metrics_summary: list[dict[str, Any]]
    ) -> dict[str, Any]:
        ...
        two_of_three_totals = [
            p["sentence_level_support"]["two_of_three_passed"]
            for p in precision_metrics_summary
        ]
        evidence_jaccard_improvements = [
            p["sentence_level_support"]["evidence_jaccard_improved"]
            for p in precision_metrics_summary
        ]
        evidence_coverage_improvements = [
            p["sentence_level_support"]["evidence_coverage_improved"]
            for p in precision_metrics_summary
        ]

        # Hard gates analysis
        numeric_matches = [
            p["hard_gates"]["numeric_matches"] for p in precision_metrics_summary
        ]
        entity_matches = [
            p["hard_gates"]["entity_matches"] for p in precision_metrics_summary
        ]
        penalties_applied = [
            p["hard_gates"]["penalty_applied"] for p in precision_metrics_summary
        ]

        # Redundancy reduction analysis
        trigram_pruned = [
            p["redundancy_reduction"]["trigram_pruned"]
            for p in precision_metrics_summary
        ]
        per_chunk_capped = [
            p["redundancy_reduction"]["per_chunk_capped"]
            for p in precision_metrics_summary
        ]
        unique_anchors_required = [
            p["redundancy_reduction"]["unique_anchors_required"]
            for p in precision_metrics_summary
        ]

        # Claim binding analysis
        claims_kept = [
            p["claim_binding"]["claims_kept"] for p in precision_metrics_summary
        ]
        min_words_met = [
            p["claim_binding"]["min_words_met"] for p in precision_metrics_summary
        ]
        strong_cases_boosted = [
            p["claim_binding"]["strong_cases_boosted"]
            for p in precision_metrics_summary
        ]

        # Facet influence analysis
        facet_downweighted = [
            p["facet_influence"]["facet_downweighted"]
            for p in precision_metrics_summary
        ]
        high_yield_facets_kept = [
            p["facet_influence"]["high_yield_facets_kept"]
            for p in precision_metrics_summary
        ]
        ...
        return {
            "sentence_level_support": {
                "avg_two_of_three_passed": (
                    sum(two_of_three_totals) / len(two_of_three_totals)
                    if two_of_three_totals
                    else 0.0
                ),
                "avg_evidence_jaccard_improvement": (
                    sum(evidence_jaccard_improvements)
                    / len(evidence_jaccard_improvements)
                    if evidence_jaccard_improvements
                    else 0.0
                ),
                "avg_evidence_coverage_improvement": (
                    sum(evidence_coverage_improvements)
                    / len(evidence_coverage_improvements)
            ...
            "hard_gates": {
                ...
                "avg_penalty_applied": (
                    sum(penalties_applied) / len(penalties_applied)
            ...
            "redundancy_reduction": {
                ...
                "avg_unique_anchors_required": (
                    sum(unique_anchors_required) / len(unique_anchors_required)
                    if unique_anchors_required
                    else 0.0
                ),
            },
            "claim_binding": {
                "avg_claims_kept": (
                    sum(claims_kept) / len(claims_kept) if claims_kept else 0.0
                ),
                "avg_min_words_met": (
                    sum(min_words_met) / len(min_words_met) if min_words_met else 0.0
                ),
            ...
            "facet_influence": {
                ...
                "avg_high_yield_facets_kept": (
                    sum(high_yield_facets_kept) / len(high_yield_facets_kept)
                    if high_yield_facets_kept
                    else 0.0
                ),
            },
        }
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="RAGChecker Precision Lift Pack Evaluation"
    )
    parser.add_argument(
        "--output",
        type=str,
    ...
    print(
        f"   Precision Lift Statistics: {evaluation_report['precision_lift_statistics']}"
    )
    ...
    print(
        f"   Two-of-Three Support: {precision_stats['sentence_level_support']['avg_two_of_three_passed']:.1f} avg"
    )
    print(
    ...
    print(
        f"   Claims Optimized: {precision_stats['claim_binding']['avg_claims_kept']:.1f} avg kept"
    )
    print(
        f"   Facet Influence: {precision_stats['facet_influence']['total_facet_downweighted']} downweighted"
    )
