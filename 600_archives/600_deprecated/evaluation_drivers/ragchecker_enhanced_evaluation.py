...
class EnhancedRAGCheckerEvaluator:
    ...
    def evaluate_query(
        ...
        try:
            if enable_enhanced_features:
                ...
                retrieved_chunks = self.pipeline.retrieve_with_context(
                    query, query_type
                )

                # Generate enhanced answer
                result = self.generator.generate_enhanced_answer(
                ...
                enhanced_metrics = {
                    ...
                    "meets_citation_requirement": validation[
                        "meets_citation_requirement"
                    ],
    ...
    def run_enhanced_evaluation(
        ...
        for i, test_case in enumerate(test_cases):
            logger.info(
                f"Evaluating case {i+1}/{len(test_cases)}: {test_case.get('name', f'case_{i+1}')}"
            )
            ...
            result = self.evaluate_query(
                query, query_type, enable_enhanced_features=True
            )
            ...
            if result["enhanced_metrics"]["abstention"]:
                logger.info(
                    f"  ⚠️  Abstention: {result['enhanced_metrics']['abstention_reason']}"
                )
            else:
                logger.info(
                    f"  📊 Citations: {result['enhanced_metrics']['citations_count']}"
                )
    ...
    def _calculate_enhanced_summary(
        ...
        context_sufficient = sum(
            1 for r in results if r["enhanced_metrics"]["has_sufficient_context"]
        )
        context_sufficiency_rate = (
            context_sufficient / total_cases if total_cases > 0 else 0
        )

        citation_requirement_met = sum(
            1 for r in results if r["enhanced_metrics"]["meets_citation_requirement"]
        )
        citation_success_rate = (
            citation_requirement_met / total_cases if total_cases > 0 else 0
        )
...
def main():
    ...
    test_cases = [
        ...
        {
            "name": "memory_system_001",
            "query": "Explain the LTST memory system architecture",
            "type": "explanatory",
        },
        {
    ...
    print(
        f"  🔍 Context Sufficiency: {enhanced_metrics['context_sufficiency_rate']:.1%}"
    )
