...
class RAGCheckerResult(BaseModel):
    ...
    custom_score: float = Field(
        ..., ge=0.0, le=1.0, description="Custom evaluation score (0-1)"
    )
    ragchecker_scores: dict[str, float] = Field(
        ..., description="Dictionary of RAGChecker metric scores"
    )
    ragchecker_overall: float = Field(
        ..., ge=0.0, le=1.0, description="Overall RAGChecker score (0-1)"
    )
    comparison: dict[str, Any] = Field(
        ..., description="Comparison data between custom and RAGChecker scores"
    )
    recommendation: str = Field(
        ..., min_length=1, description="Recommendation based on evaluation results"
    )
    ...
    def validate_ragchecker_scores(cls, v):
        ...
        for score_name, score_value in v.items():
            if (
                ...
                raise ValueError(
                    f"Score {score_name} must be between 0.0 and 1.0, got {score_value}"
                )
        return v
...
class RAGCheckerEvaluator:
    ...
    def __init__(self):
        ...
        self.validation_optimizer = create_validation_optimizer(
            enable_caching=True, enable_batching=True
        )
        ...
        try:
            ...
            if retrieved_context:
                retrieved_docs.append(
                    RetrievedDoc(doc_id="memory_context", text=retrieved_context)
                )
    ...
    def _calculate_basic_metrics(self, rag_result: RAGResult) -> dict[str, float]:
        ...
        metrics["response_length"] = min(
            response_length / 1000, 1.0
        )  # Normalize to 0-1
    ...
    def validate_with_constitution(
        ...
        debug_context = self.debug_manager.capture_ragchecker_context(
            ...
            evaluation_type="constitution_validation",
        ...
        try:
            ...
            if "input" in evaluation_data:
                ...
                input_validation = (
                    self.constitution_validator.validate_ragchecker_input(
                        evaluation_data["input"]
                    )
                )
                ...
                if not input_validation["valid"]:
                    ...
                    validation_results["total_violations"] += len(
                        input_validation["errors"]
                    )
                validation_results["total_warnings"] += len(
                    input_validation.get("warnings", [])
                )
                ...
                self.debug_manager.log_performance_metrics(
                    context=debug_context,
                    metrics={"input_validation_time": validation_time},
                )
                ...
                self.performance_monitor.record_operation(
                    ...
                    metadata={
                        ...
                        "compliance_score": self._extract_compliance_score(
                            input_validation
                        ),
                    },
                )
            ...
            if "metrics" in evaluation_data:
                start_time = time.time()
                metrics_validation = (
                    self.constitution_validator.validate_ragchecker_metrics(
                ...
                validation_results["total_warnings"] += len(
                    metrics_validation.get("warnings", [])
                )
                ...
                self.debug_manager.log_performance_metrics(
                    context=debug_context,
                    metrics={"metrics_validation_time": validation_time},
                )
            ...
            if "result" in evaluation_data:
                start_time = time.time()
                result_validation = (
                    self.constitution_validator.validate_ragchecker_result(
                ...
                validation_results["total_warnings"] += len(
                    result_validation.get("warnings", [])
                )
                ...
                self.debug_manager.log_performance_metrics(
                    context=debug_context,
                    metrics={"result_validation_time": validation_time},
                )
            ...
            validation_results["constitution_summary"] = {
                ...
                "compliance_score": (
                    1.0 if validation_results["overall_compliance"] else 0.3
                ),
            ...
            self.debug_manager.log_constitution_validation(
                ...
                compliance_score=validation_results["constitution_summary"][
                    "compliance_score"
                ],
            )
            ...
            if "cache_hit_rate" in optimizer_summary:
                self.performance_monitor.update_metrics(
                    {"cache_hit_rate": optimizer_summary["cache_hit_rate"]}
                )
    ...
    def validate_with_constitution_and_taxonomy(
        self, evaluation_data: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        enhanced_results = self.constitution_validator.enhance_validation_with_taxonomy(
            validation_results
        )
    ...
    def recover_from_validation_error(
        ...
        return self.error_recovery.recover_from_error(
            error=error, error_type=error_type, context=context
        )
    ...
    def get_performance_history(
        ...
        return self.performance_monitor.get_performance_history(
            start_time, end_time, max_points
        )

    def export_performance_metrics(
        self, filepath: str, format_type: str = "json"
    ) -> bool:
    ...
    def calculate_ragchecker_overall(
        self, ragchecker_scores: dict[str, float]
    ) -> float:
    ...
    def compare_evaluations(
        self, custom_score: float, ragchecker_overall: float
    ) -> dict[str, Any]:
    ...
    def evaluate_test_case(
        self, case_name: str, query: str, role: str = "planner"
    ) -> RAGCheckerResult:
        ...
        try:
            ...
            custom_result = self.official_evaluator.create_fallback_evaluation(
                fallback_data
            )
    ...
    def run_ragchecker_evaluation_suite(self) -> dict[str, Any]:
        ...
        ragchecker_avg = (
            ragchecker_total / len(baseline_cases) if baseline_cases else 0.0
        )
        ...
        output_file = (
            f"metrics/baseline_evaluations/ragchecker_evaluation_{timestamp}.json"
        )
...
def main():
    ...
    print(
        f"Based on the comparison between custom evaluation ({results['custom_average']:.1f}/100)"
    )
    ...
    print(
        "   For full RAGChecker capabilities, configure LLM access (OpenAI, Bedrock, etc.)"
    )
