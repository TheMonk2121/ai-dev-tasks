...
class RecoveryStrategy(BaseModel):
    ...
    retry_delay: float = Field(
        default=0.1, ge=0.0, le=5.0, description="Delay between retries in seconds"
    )
    ...
    recovery_timeout: float = Field(
        default=30.0, ge=1.0, le=300.0, description="Maximum recovery time in seconds"
    )
...
class ErrorRecoveryContext(BaseModel):
    ...
    start_time: datetime = Field(
        default_factory=datetime.now, description="Recovery start time"
    )
    recovery_steps: list[str] = Field(
        default_factory=list, description="Recovery steps attempted"
    )
    fallback_used: bool = Field(default=False, description="Whether fallback was used")
    recovery_successful: bool = Field(
        default=False, description="Whether recovery was successful"
    )
...
class RAGCheckerErrorRecovery:
    ...
    def _register_default_strategies(self):
        ...
        self.register_recovery_strategy(
            "pydantic_validation", self._recover_pydantic_validation
        )
        self.register_recovery_strategy(
            "constitution_validation", self._recover_constitution_validation
        )
        self.register_recovery_strategy(
            "error_taxonomy_mapping", self._recover_error_taxonomy_mapping
        )
        self.register_recovery_strategy(
            "performance_degradation", self._recover_performance_degradation
        )
        self.register_recovery_strategy(
            "memory_query_failure", self._recover_memory_query_failure
        )

    def register_recovery_strategy(
        self, error_type: str, strategy_func: Callable
    ) -> None:
    ...
    def recover_from_error(
        ...
        try:
            ...
            if error_type in self.recovery_strategies:
                ...
                result = self._execute_recovery_with_retry(
                    recovery_func, error, context, recovery_context
                )
            else:
                # Use generic recovery
                result = self._execute_generic_recovery(
                    error, context, recovery_context
                )
            ...
            self.logger.error(
                f"Error recovery failed for {error_type}: {str(recovery_error)}"
            )

            # Try fallback if enabled
            if strategy.fallback_enabled:
                result = self._execute_fallback_recovery(
                    error, context, recovery_context
                )
            else:
                result = {
    ...
    def _execute_recovery_with_retry(
        ...
        for attempt in range(strategy.max_retries + 1):
            try:
                ...
                recovery_context.recovery_steps.append(
                    f"Attempt {attempt + 1}: {recovery_func.__name__}"
                )

                # Check timeout
                elapsed_time = (
                    datetime.now() - recovery_context.start_time
                ).total_seconds()
                ...
                if attempt < strategy.max_retries:
                    delay = strategy.retry_delay * (
                        strategy.backoff_multiplier**attempt
                    )
                    self.logger.info(
                        f"Recovery attempt {attempt + 1} failed, retrying in {delay:.2f}s"
                    )
                    time.sleep(delay)
                ...
                recovery_context.recovery_steps.append(
                    f"Attempt {attempt + 1} failed: {str(retry_error)}"
                )
    ...
    def _execute_generic_recovery(
        ...
        recovery_context.recovery_steps.append(
            "Generic recovery: error logging and context preservation"
        )
    ...
    def _execute_fallback_recovery(
        ...
        recovery_context.recovery_steps.append(
            "Fallback recovery: graceful degradation"
        )
    ...
    def _recover_pydantic_validation(
        ...
        recovery_context.recovery_steps.append(
            "Pydantic validation recovery: data sanitization"
        )
        ...
        if isinstance(error, ValidationError):
            ...
            try:
                ...
                fixed_data = self._fix_validation_errors(
                    context.get("data", {}), field_errors
                )
    ...
    def _recover_constitution_validation(
        ...
        recovery_context.recovery_steps.append(
            "Constitution validation recovery: rule relaxation"
        )
    ...
    def _recover_error_taxonomy_mapping(
        ...
        recovery_context.recovery_steps.append(
            "Error taxonomy recovery: fallback categorization"
        )
    ...
    def _recover_performance_degradation(
        ...
        recovery_context.recovery_steps.append(
            "Performance recovery: optimization mode"
        )
    ...
    def _recover_memory_query_failure(
        ...
        recovery_context.recovery_steps.append(
            "Memory query recovery: fallback data sources"
        )
    ...
    def _fix_validation_errors(
        self, data: dict[str, Any], field_errors: list[dict[str, Any]]
    ) -> dict[str, Any]:
                ...
                if error_type == "missing":
                    ...
                    if field_path == "custom_score":
                        ...
                        if not isinstance(context, list):
                            fixed_data[field_path] = (
                                [str(context)] if context else ["Default context"]
                            )
