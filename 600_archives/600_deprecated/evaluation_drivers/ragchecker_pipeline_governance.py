...
spec = importlib.util.spec_from_file_location(
    ...
    str(
        ...
        / "300_experiments"
        / "300_rag_pipeline_governance.py"
    ),
)
...
class RAGCheckerPipelineGovernance:
    ...
    def _initialize_known_good_patterns(self):
        ...
        standard_id = self.governance.create_pipeline_graph(
            standard_pipeline, "ragchecker_standard"
        )
        ...
        enhanced_id = self.governance.create_pipeline_graph(
            enhanced_pipeline, "ragchecker_enhanced"
        )
        self.governance.known_good_patterns.add(enhanced_id)

        logger.info(
            f"Initialized {len(self.governance.known_good_patterns)} known good pipeline patterns"
        )

    def validate_ragchecker_pipeline(
        ...
        if is_unusual or not validation_results["valid"]:
            ...
            if suggested_variant:
                ...
                validation_results["suggestions"].append(
                    f"Consider using known-good variant: {suggested_variant}"
                )
    ...
    def optimize_ragchecker_pipeline(
    ...
    def generate_pipeline_variants(
        self, base_config: dict[str, Any], num_variants: int = 5
    ) -> list[dict[str, Any]]:
        ...
        for i in range(num_variants - len(variants)):
            ...
            variant_config = self.governance.pipeline_metadata[variant_id]["config"]
            variants.append(
                {"config": variant_config, "type": "semantic", "base_pipeline": base_id}
            )
    ...
    def _simulate_ragchecker_evaluation(
        self, query: str, pipeline_config: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        chunk_size = (
            pipeline_config.get("chunk", {})
            .get("parameters", {})
            .get("chunk_size", 512)
        )
        top_k = (
            pipeline_config.get("retrieve", {}).get("parameters", {}).get("top_k", 5)
        )
        temperature = (
            pipeline_config.get("generate", {})
            .get("parameters", {})
            .get("temperature", 0.7)
        )
    ...
    def _calculate_requirement_match(
        self, requirements: dict[str, Any], metadata: dict[str, Any]
    ) -> float:
        ...
        if "high_precision" in requirements and requirements["high_precision"]:
            ...
            chunk_size = (
                config.get("chunk", {}).get("parameters", {}).get("chunk_size", 512)
            )
...
if __name__ == "__main__":
    ...
    print(
        f"Governance report: {report['governance_system']['total_pipelines']} pipelines managed"
    )
