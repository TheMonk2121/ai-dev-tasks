...
class RAGCheckerDebugContext(PydanticModel):
    ...
    evaluation_type: str = PydField(
        ..., description="Type of evaluation (input, metrics, result)"
    )
    ...
    error_taxonomy_context: dict[str, Any] = PydField(
        default_factory=dict, description="Error taxonomy context"
    )
    performance_metrics: dict[str, Any] = PydField(
        default_factory=dict, description="Performance metrics"
    )
    timestamp: datetime = PydField(
        default_factory=datetime.now, description="Context timestamp"
    )
...
class RAGCheckerDebugManager:
    ...
    def __init__(self, enable_privacy: bool = True, capture_variables: bool = True):
        ...
        if EnhancedDebuggingManager is not None:
            self.debug_manager = EnhancedDebuggingManager(
                enable_privacy=enable_privacy, max_context_history=100
            )
    ...
    def capture_ragchecker_context(
        ...
        **context_data,
        ...
        # Log context capture
        self.logger.debug(
            f"Captured RAGChecker context: {evaluation_id} - {evaluation_type} - {validation_stage}"
        )
    ...
    def log_error_taxonomy_mapping(
        ...
        categorization_success_rate: float,
    ) -> None:
    ...
    def log_performance_metrics(
        self, context: RAGCheckerDebugContext, metrics: dict[str, float]
    ) -> None:
    ...
    def log_validation_error(
        ...
        if self.enhanced_debugging_enabled and self.debug_manager:
            ...
            rich_error = self.debug_manager.create_rich_error_message(
                error=error,
                debugging_context=self._convert_to_debugging_context(context),
            )
    ...
    def _convert_to_debugging_context(
        self, context: RAGCheckerDebugContext
    ) -> Any | None:
...
def create_ragchecker_debug_manager(
    ...
    return RAGCheckerDebugManager(
        enable_privacy=enable_privacy, capture_variables=capture_variables
    )
...
if __name__ == "__main__":
    ...
    context = debug_manager.capture_ragchecker_context(
        ...
        evaluation_type="input_validation",
        validation_stage="pydantic_validation",
    )
