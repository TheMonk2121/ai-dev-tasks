...
class FinalRAGASPushEvaluator:
    ...
    def run_final_ragas_push_evaluation(self) -> dict[str, Any]:
        ...
        if self.config_manager:
            ...
            print(
                f"📊 Applied all moves: {sum(len(config) for config in applied_configs.values())} parameters"
            )
        ...
        if OfficialRAGCheckerEvaluator:
            ...
            results = base_evaluator.run_official_evaluation(
                use_local_llm=True, local_api_base=None, use_bedrock=False
            )
        else:
        ...
        if self.config_manager:
            validation = self.config_manager.validate_targets(
...
def main():
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="final_ragas_push_results.json",
