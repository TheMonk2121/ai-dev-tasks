...
class EnhancedRAGCheckerWithLimitFeatures(OfficialRAGCheckerEvaluator):
    ...
    def __init__(self):
        ...
        print(
            "📊 Geometry router, facet selector, boolean parser, and support validator ready"
        )
    ...
    def _init_geometry_router(self):
        ...
        return {
            "margin_threshold": float(
                os.getenv("RAGCHECKER_ROUTE_BM25_MARGIN", "0.20")
            ),
            "agreement_threshold": float(
                os.getenv("RAGCHECKER_REWRITE_AGREE_STRONG", "0.50")
            ),
        }
    ...
    def _init_support_validator(self):
        ...
        return {
            ...
            "evidence_coverage": float(
                os.getenv("RAGCHECKER_EVIDENCE_COVERAGE", "0.20")
            ),
            "cosine_floor": float(os.getenv("RAGCHECKER_COSINE_FLOOR", "0.58")),
            "numeric_must_match": os.getenv("RAGCHECKER_NUMERIC_MUST_MATCH", "1")
            == "1",
            "entity_must_match": os.getenv("RAGCHECKER_ENTITY_MUST_MATCH", "1") == "1",
        }

    def get_memory_system_response_with_limit_features(
        self, query: str, role: str = "planner"
    ) -> str:
        ...
        try:
            ...
            retrieved_docs = self._hybrid_retrieval_with_geometry_routing(
                query, boolean_logic, facets
            )
    ...
    def _hybrid_retrieval_with_geometry_routing(
        self,
        query: str,
        boolean_logic: dict[str, list[str]],
        ...
        print(
            f"[router] margin={margin:.3f} entropy={entropy:.3f} agree={rewrite_agreement:.2f} → {route}"
        )
    ...
    def _simulate_bm25_retrieval(
        self, query: str, boolean_logic: dict[str, list[str]]
    ) -> list[dict[str, Any]]:
    ...
    def _simulate_hybrid_retrieval(
        ...
        for doc in fused_docs:
            if not doc["has_query_anchors"]:
                doc["score"] *= float(
                    os.getenv("RAGCHECKER_FACET_DOWNWEIGHT_NO_ANCHOR", "0.80")
                )
            else:
    ...
    def _apply_mmr_diversification(
        ...
        for doc in docs:
            ...
            content_hash = hash(
                doc["content"][:50]
            )  # Use first 50 chars as similarity proxy
    ...
    def _enforce_per_doc_line_cap(
        self, docs: list[dict[str, Any]], cap: int
    ) -> list[dict[str, Any]]:
    ...
    def _apply_support_validation(
        ...
        print(
            f"[support] {len(docs)} → {len(validated_docs)} docs passed two-of-three + validation"
        )
    ...
    def _generate_enhanced_response(
        self, query: str, validated_docs: list[dict[str, Any]], role: str
    ) -> str:
...
Please provide a comprehensive response that utilizes this enhanced context."""
    ...
    def create_fallback_evaluation_with_limit_features(
        ...
        for case in input_data:
            ...
            enhanced_response = self.get_memory_system_response_with_limit_features(
                case["query"]
            )

            # Calculate metrics using enhanced response
            precision = self.calculate_precision(
                enhanced_response, case["gt_answer"], case["query"]
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Enhanced RAGChecker with LIMIT Features"
    )
    parser.add_argument(
        "--test-query",
        type=str,
        default="DSPy integration patterns",
        help="Test query to evaluate",
    )
    parser.add_argument(
    ...
    enhanced_response = evaluator.get_memory_system_response_with_limit_features(
        args.test_query
    )
    ...
    print(
        f"   Geometry Router: margin_threshold={features['geometry_router']['margin_threshold']}"
    )
    print(f"   Facet Selector: min_yield={features['facet_selector']['min_yield']}")
    print(
        f"   Support Validator: evidence_jaccard={features['support_validator']['evidence_jaccard']}"
    )
