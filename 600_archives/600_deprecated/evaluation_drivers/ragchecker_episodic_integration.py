...
class RAGCheckerEpisodicIntegration:
    ...
    def enhance_query_with_episodic_context(
        self, query: str, agent: str = "cursor_ai"
    ) -> dict[str, Any]:
        ...
        try:
            ...
            if context and "episodic_context" in context:
                ...
                print(
                    f"✅ Enhanced query with episodic context (confidence: {episodic['confidence_score']:.2f})"
                )
    ...
    def _create_enhanced_query(
        self, original_query: str, episodic_context: dict[str, Any]
    ) -> str:
    ...
    def store_evaluation_result(
        ...
        try:
            ...
            task_description = (
                f"RAGChecker evaluation with episodic context: {query[:50]}..."
            )
    ...
    def run_ablation_study(
        self, queries: list, agent: str = "cursor_ai"
    ) -> dict[str, Any]:
...
def main():
    ...
    parser.add_argument(
        "command", choices=["enhance", "ablation", "test"], help="Command to execute"
    )
    parser.add_argument("--query", help="Query to enhance")
    parser.add_argument(
        "--queries-file", help="File containing queries for ablation study"
    )
    ...
    if args.command == "enhance":
        ...
        enhanced_query = integration.enhance_query_with_episodic_context(
        ...
        if args.format == "json":
            ...
            if enhanced_query["episodic_context"]:
                ...
                print(
                    f"   What worked: {enhanced_query['episodic_context']['what_worked']}"
                )
                print(
                    f"   What to avoid: {enhanced_query['episodic_context']['what_to_avoid']}"
                )
        ...
        try:
            ...
            if args.format == "json":
                ...
                print(
                    f"   Enhanced queries: {results['with_episodic']}/{results['total_queries']}"
                )
        ...
        enhanced_query = integration.enhance_query_with_episodic_context(
            test_query, args.agent
        )

        if enhanced_query["episodic_context"]:
            print("✅ Query enhancement test passed")
            print(
                f"   Enhanced query length: {len(enhanced_query['enhanced_query'])} chars"
            )
            print(
                f"   Confidence score: {enhanced_query['episodic_context']['confidence_score']:.2f}"
            )
        else:
