...
class PrecisionOptimizedRAGChecker(EnhancedRAGCheckerWithLimitFeatures):
    ...
    def _apply_precision_optimization(self):
        ...
        self.geometry_router["margin_threshold"] = float(
            precision_config["RAGCHECKER_ROUTE_BM25_MARGIN"]
        )
        self.geometry_router["agreement_threshold"] = float(
            precision_config["RAGCHECKER_REWRITE_AGREE_STRONG"]
        )
        self.facet_selector["max_facets"] = int(
            precision_config["RAGCHECKER_REWRITE_K"]
        )
        self.facet_selector["keep_facets"] = int(
            precision_config["RAGCHECKER_REWRITE_KEEP"]
        )
        self.facet_selector["min_yield"] = float(
            precision_config["RAGCHECKER_REWRITE_YIELD_MIN"]
        )
        self.support_validator["evidence_jaccard"] = float(
            precision_config["RAGCHECKER_EVIDENCE_JACCARD"]
        )
        self.support_validator["evidence_coverage"] = float(
            precision_config["RAGCHECKER_EVIDENCE_COVERAGE"]
        )
    ...
    def _apply_support_validation(
        ...
        print(
            f"[precision_support] {len(docs)} → {len(validated_docs)} docs passed strict validation"
        )
        return validated_docs

    def _validate_numeric_entity_match_strict(
    ...
    def _simulate_hybrid_retrieval(
        ...
        for i in range(12):  # Reduced from 16
            doc = {
                ...
                "has_query_anchors": np.random.choice(
        ...
        for doc in fused_docs:
            if not doc["has_query_anchors"]:
                doc["score"] *= float(
                    os.getenv("RAGCHECKER_FACET_DOWNWEIGHT_NO_ANCHOR", "0.70")
                )
            else:
...
def main():
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="precision_optimized_output.json",
