...
class RAGCheckerGovernanceIntegration:
    ...
    def evaluate_with_governance(
        ...
        validation_results = self.governance.validate_ragchecker_pipeline(
            pipeline_config
        )

        if not validation_results["valid"]:
            logger.warning("⚠️ Pipeline validation failed, attempting optimization...")
            optimized_config = self.governance.optimize_ragchecker_pipeline(
                pipeline_config
            )
        ...
        try:
            ...
            evaluation_results = self.ragchecker.run_official_evaluation(
                use_local_llm=False, use_bedrock=use_bedrock
            )
            ...
            for metric, target in performance_targets.items():
                ...
                logger.info(
                    f"📊 {metric}: {actual:.3f} (target: {target:.3f}) {status}"
                )
    ...
    def evaluate_pipeline_variants(
        ...
        for i, variant in enumerate(variants):
            logger.info(
                f"🧪 Evaluating variant {i+1}/{len(variants)} ({variant['type']})"
            )

            try:
                # Run evaluation for this variant
                result = self.evaluate_with_governance(
                    pipeline_config=variant["config"],
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Generate and evaluate N pipeline variants",
    )
    parser.add_argument(
        "--use-bedrock", action="store_true", help="Use Bedrock for evaluation"
    )
    ...
    if args.variants > 0:
        ...
        logger.info(
            f"🔄 Generating and evaluating {args.variants} pipeline variants..."
        )
        ...
        results = integration.evaluate_with_governance(
            use_bedrock=args.use_bedrock, num_cases=args.num_cases
        )
