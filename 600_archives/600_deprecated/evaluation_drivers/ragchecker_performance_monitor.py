...
class PerformanceThresholds(PydBaseModel):
    ...
    max_execution_time: float = PydField(
        default=1.0, description="Maximum execution time in seconds"
    )
    min_throughput: float = PydField(
        default=100.0, description="Minimum throughput in ops/sec"
    )
    max_error_rate: float = PydField(
        default=5.0, description="Maximum error rate percentage"
    )
    max_memory_usage: float = PydField(
        default=500.0, description="Maximum memory usage in MB"
    )
    min_cache_hit_rate: float = PydField(
        default=80.0, description="Minimum cache hit rate percentage"
    )
    max_response_time: float = PydField(
        default=2.0, description="Maximum response time in seconds"
    )
...
class PerformanceMonitor:
    ...
    def _check_performance_thresholds(self):
        ...
        if (
            self.current_metrics.get("cache_hit_rate", 0)
    ...
    def get_performance_summary(self) -> dict[str, Any]:
        ...
        if len(self.performance_history) > 1:
            ...
            time_diff = (
                latest_snapshot.timestamp - previous_snapshot.timestamp
            ).total_seconds()

            throughput_trend = (
                (latest_snapshot.throughput - previous_snapshot.throughput) / time_diff
                if time_diff > 0
                else 0.0
            )
            error_rate_trend = (
                (latest_snapshot.error_rate - previous_snapshot.error_rate) / time_diff
                if time_diff > 0
                else 0.0
            )
        else:
...
if __name__ == "__main__":
    ...
    monitor.record_operation(
        "test_operation", 0.05, success=False, error_type="ValueError"
    )
