#!/usr/bin/env python3
"""
DEPRECATED: Improved historical evaluation data migration to TimescaleDB.

This file has been moved to 600_archives and is deprecated.
Use scripts/migration/load_historical_evals.py instead.

This script implements best practices for migrating JSON evaluation data:
...
class ImprovedHistoricalEvalMigrator:
    """Improved migrator with proper batch processing and error handling."""
    ...
    def __enter__(self):
        ...
        self.conn = psycopg.connect(resolve_dsn(strict=False))
        ...
        try:
            ...
            if not self.dry_run:
                self.cur.execute(
                    ...
                    VALUES (%(run_id)s, %(tag)s, %(started_at)s, %(finished_at)s, %(model)s, %(meta)s)
            ...
            for i, case in enumerate(case_results):
                ...
                if not self.dry_run:
                    self.cur.execute(
                        ...
                        VALUES (%(run_id)s, %(case_id)s, %(f1)s, %(precision)s, %(recall)s, %(latency_ms)s, %(ok)s, %(meta)s)
    ...
    def migrate_all_evaluations(self, base_dir: str, limit: int | None = None) -> dict[str, int]:
        ...
        print("🚀 Starting improved migration of historical evaluation data...")
...
def main():
    ...
    with ImprovedHistoricalEvalMigrator(batch_size=args.batch_size, dry_run=args.dry_run) as migrator:
