...
def simulate_mission_execution(mission_id: str, agent_type: str = None, model_used: str = None):
    ...
    for progress in range(0, 101, random.randint(5, 15)):
        ...
        mission_tracker.update_mission_progress(mission_id=mission_id, progress=progress, result=result)
    ...
    success = mission_tracker.complete_mission(
        mission_id=mission_id, result=final_result, tokens_used=estimated_tokens, cost_estimate=estimated_cost
    )
...
def simulate_mission_failure(mission_id: str, error_message: str):
    ...
    # Start the mission
    success = mission_tracker.start_mission(
        mission_id=mission_id, agent_type="IntentRouter", model_used="mistral:7b-instruct"
    )
    ...
    # Fail the mission
    success = mission_tracker.fail_mission(mission_id=mission_id, error_message=error_message)
...
def run_demo():
    ...
    for i, mission_id in enumerate(mission_ids[:3]):
        threading.Thread(target=simulate_mission_execution, args=(mission_id,), daemon=True).start()
    ...
    time.sleep(3)
    threading.Thread(
        target=simulate_mission_failure, args=(mission_ids[6], "Database connection timeout"), daemon=True
    ).start()

    # Start the last mission
    time.sleep(2)
    threading.Thread(target=simulate_mission_execution, args=(mission_ids[7],), daemon=True).start()
