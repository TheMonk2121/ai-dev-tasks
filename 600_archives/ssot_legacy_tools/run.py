...
LATEST_DIR = Path("evals/metrics/latest")
LATEST_DIR.mkdir(parents=True, exist_ok=True)

app = typer.Typer()


def _load_layer(name: str) -> dict[str, str]:  # pyright: ignore[reportAny]
    env = {}
...
def _materialize_env(layers: list[str], overrides: dict[str, Any]) -> dict[str, str]:  # pyright: ignore[reportAny]
    env: dict[str, str] = {}
    for layer in layers:
        env.update(_load_layer(layer))
    env.update({k: str(v) for k, v in os.environ.items()})
    # Guardrails: keep stable behavior unless explicit
    if not env.get("MAX_WORKERS"):
        env.setdefault("MAX_WORKERS", "3")
    return env


def _write_latest_metrics(pass_id: str, payload: dict[str, Any]) -> None:  # pyright: ignore[reportAny]
...
def run(suite: str, pass_: str):
    ...
    subprocess.run(["python", "-m", "evals.tools.report"], check=True)
