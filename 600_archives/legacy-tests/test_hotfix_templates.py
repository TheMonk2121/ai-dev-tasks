...
from utils.hotfix_templates import (
    HotFixGenerator, 
    generate_hotfix_template, 
    list_hotfix_templates,
    HotFixTemplate
)
...
class TestHotFixTemplates:
    ...
    def test_hotfix_template_variables(self):
        ...
        for template in templates:
            ...
            for variable in template.variables:
                assert f"{{{variable}}}" in template.template_content, \
    ...
    def test_hotfix_template_categories(self):
        ...
        for category in expected_categories:
            assert any(template.category == category for template in templates), \
    ...
    def test_hotfix_template_prerequisites(self):
        ...
        for template in templates:
            assert len(template.prerequisites) > 0, \
                f"Template {template.name} has no prerequisites"
