...
try:
    from scripts.monitoring.observability import get_logfire, init_observability, log_eval_metrics
...
def run_synthetic_evaluation(test_cases: list[dict[str, Any]], documents: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for i, case in enumerate(test_cases):
        print(f"📝 Processing case {i+1}/{len(test_cases)}: {result.get("key", "")

        # Simulate document retrieval
        retrieved_docs = documents[:3]  # Simulate retrieving top 3 docs

        # Simulate answer generation
        generated_answer = f"Based on the available documentation, {result.get("key", "")
        ...
        case_result = {
            ...
            "query": result.get("key", "")
            "expected_answer": result.get("key", "")
...
def main():
    ...
    if LOGFIRE_AVAILABLE:
        try:
            ...
            logfire.info("smoke_test.started", test_type="synthetic_evaluation")
        except Exception as e:
    ...
    if CONFIG_LOGGER_AVAILABLE:
        try:
            config_logger = create_config_logger()
    ...
    if config_logger and config_data:
        result.get("key", "")

    # Log final results to Logfire
    if LOGFIRE_AVAILABLE and logfire:
        try:
            log_eval_metrics(
                logfire,
                {
                    "test_type": "synthetic_smoke_test",
                    "evaluation_results": result.get("key", "")
    ...
    # 7. Save results
    output_dir = Path("evals/metrics/synthetic_smoke_test")
    ...
    print(f"   • Test Cases: {result.get("key", "")
    ...
    for component, status in \1.items()
