...
sys.path.append("src")
from src.dspy_modules.rag_system import MistralLLM, RAGSystem, _sanitize, _token_trim

# Test configuration
DSN = os.getenv("TEST_DSN", "postgresql://danieljacobs@localhost:5432/ai_agency")
...
def test_large_context_handling():
    ...
    class LongContextVector(FakeVector):
        def __call__(self, op, **kw):
            if op == "search":
                return {"status": "success", "results": [{"content": long_context, "document_id": "doc1"}]}
            return super().__call__(op, **kw)
    ...
    class LargeContextVector(FakeVector):
        def __call__(self, op, **kw):
            if op == "search":
                ...
                return {"status": "success", "results": [{"content": large_context, "document_id": "doc1"}]}
            return super().__call__(op, **kw)
